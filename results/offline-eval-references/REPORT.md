...
- **Tool:** `scripts/eval_checkpoint.py` (from `build-offline-eval`), driven by
  `run_offline_eval.sh`.
...
- **Fused, no eager (NON-NEGOTIABLE #1):**
  - emender: `level=E97`, `use_triton=1` → fused split-edit Triton kernel (the forward
    hard-imports the kernel and raises rather than falling back to eager).
  - gdn2: `level=gdn2-mlp` → `GDN2ExternalMLPLayer` with `mode="chunk"`, the FLA chunked
    GDN-2 fused Triton kernel (`GDN2_PATH=/home/erikg/GatedDeltaNet-2`).
...
- **GPU isolation:** exactly **1 idle GPU** leased via the broker (GPU 2). The two reference
  trainings on GPUs 0/1 (pids 522358, 584124) and unrelated jobs were untouched and still
  running at completion.
- **Coverage:** all 5 emender + all 5 gdn2 checkpoints present at launch were scored (the
  checkpoint inventory did not change during the run).
...
- **emender is non-monotone/noisy**: 1.4035 → 1.2839 → **1.3694** → 1.2472 → 1.2871. The
  1.3694 spike at 528 M (whose inline *filename* loss 3.1246 was the run's lowest!) and the
  1.2472 dip at 704 M are exactly the schedule-free y/x + noise wobble — the offline curve
...
|---:|---:|---:|---:|:--|
| 300 M | 1.3194 | 1.2318 | +0.0876 | **gdn2** |
| 400 M | 1.3071 | 1.1846 | +0.1225 | **gdn2** |
| 500 M | 1.3557 | 1.1594 | +0.1962 | **gdn2** |
| 600 M | 1.3198 | 1.1367 | +0.1831 | **gdn2** |
| 700 M | 1.2504 | 1.1220 | +0.1284 | **gdn2** |
| 800 M | 1.2688 | 1.1086 | +0.1602 | **gdn2** |
| 881 M | 1.2871 | 1.1000 | +0.1870 | **gdn2** |

**gdn2-mlp leads emender at every matched-token point in the overlap, by +0.088 to +0.196
BPB (~0.13–0.19 BPB through most of the range).** At the last matched point (881 M tokens):
gdn2 ≈ 1.100 vs emender 1.287, a **0.187 BPB** gdn2 lead. The gap is far larger than the
noise wobble in either curve, and gdn2 wins even against emender's *best* checkpoint in the
overlap (emender's lowest overlap point is 1.2472 @704 M, still above gdn2's 1.1220 there).

## Sanity cross-check

- **CE↔BPB conversion is consistent with the tensor's bytes_per_token.** e.g. emender step
  21500: `(3.837969 / ln2) / 3.945132 = 1.40349`, matches the reported `bpb=1.403507`. Same
  holds for every row → the held-out tensor's `bytes_per_token=3.945132` is applied
  correctly.
...
- `run_offline_eval.sh` — driver (1-GPU lease, both refs, y-mode, fused).
- `run_offline_eval.log` — full run log (per-checkpoint `schedulefree_y_swap=True`, fused level).
- `emender_heldout_bpb.csv`, `gdn2_heldout_bpb.csv` — raw per-checkpoint scores.
