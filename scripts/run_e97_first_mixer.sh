...
CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" -m pytest -q tests/test_e97_first_mixer_probe.py tests/test_e97_activation_alignment.py tests/test_e97_head_precision_probe.py tests/test_e97_outcome_rl_candidate.py tests/test_e97_native_execution.py
CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" scripts/diagnose_e97_first_mixer.py freeze --output "$RUN"
RECIPE_SHA=$(sha256sum "$RUN/recipe-private.json" | cut -d' ' -f1)
[[ $RECIPE_SHA == 84a3195ad3277650a12398ae3e023b2bacebd6995ba1d5a53a4229fd13922e88 ]]
...
timeout --kill-after=30 3600 "$PYTHON_BIN" -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node=1 --max-restarts=0 \
 scripts/numa_local_rank_exec.py scripts/diagnose_e97_first_mixer.py run \
