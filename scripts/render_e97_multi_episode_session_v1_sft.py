#!/usr/bin/env python3
"""Render the e97 multi-episode session v1 verified candidate collection as a
non-trainable masked-SFT authority (emender-e97-tulu3-masked-sft-v1) for the
E3 prep builder's session-family cohort-spec consumption.

Byte-exact copies of the audited candidate-authority token/mask/index
payloads; metadata rows gain source='multi-episode-session-rehearsal' (the
prep interleave/include-source convention). The render is bound fail-closed
to the frozen plan, the full session reconstruction audit, the
protected-panel overlap audit, AND the SmolTalk2 shingle exclusion audit.
The training_eligible key is OMITTED (the verified-raw-oh/tooltalk
precedent): read_conversation_slice consumes this authority via a
--cohort-spec entry, and eligibility flows only from the E3 preparation
admission chain, never from this render.
"""
...
def render(args):
 ...
 audit=check_receipt(args.audit,args.audit_sha,'emender-e97-multi-episode-session-audit-v1','qualified-candidates-not-admitted')
 overlap=check_receipt(args.overlap,args.overlap_sha,'emender-e97-multi-episode-session-overlap-audit-v1','pass')
 shingles=check_receipt(args.shingles,args.shingles_sha,'emender-e97-hybrid-conversation-smoltalk2-shingle-audit-v1','pass')
 if (audit['records']!=len(rows) or audit['plan_sha256']!=candidate_manifest['plan_sha256']
  or overlap['records']!=len(rows) or overlap['plan_sha256']!=candidate_manifest['plan_sha256']
  or shingles['our_records']!=len(rows)
  or shingles['our_authority_sha256']!=args.manifest_sha
  or shingles['content_collisions']!=0):raise ValueError('receipt binding')
 ...
 with outputs['tokens'].open('xb') as tf,outputs['mask'].open('xb') as mf,outputs['index'].open('xb') as ix,outputs['metadata'].open('x') as meta,paths['tokens'].open('rb') as src_t,paths['mask'].open('rb') as src_m:
  for i,row in enumerate(rows):
   ...
   out_row=dict(row);out_row['source']='multi-episode-session-rehearsal'
 ...
 manifest={'schema':'emender-e97-tulu3-masked-sft-v1','status':'complete',
  'purpose':'Non-trainable masked-SFT render of the e97 multi-episode session v1 verified candidate collection (E3 session-family cohort-spec consumption); not admitted to training',
  'admission_note':'plain data authority; training eligibility flows only from the E3 preparation admission chain (the prep manifest stays training_eligible:false)',
  'tokenizer':'p50k_base','packing_authorized':False,'optimizer_updates_authorized':0,
  'counts':{'records':records,'train_records':records,'validation_records':0,'tokens':offset,'assistant_target_tokens':targets},
  'source_target_totals':{'multi-episode-session-rehearsal':targets},
  ...
  'smoltalk2_shingle_audit_sha256':args.shingles_sha,
 ...
 print('SESSION_V1_SFT_RENDER',records,offset,targets,sha(args.output/'manifest.json'))
...
def main():
 ...
 p.add_argument('--shingles',type=Path,required=True);p.add_argument('--shingles-sha',required=True)
