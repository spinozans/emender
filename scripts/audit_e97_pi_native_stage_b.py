...
def audit(a):
 ...
 if (valid,correct,successes)!=(summary['valid_first_frames'],summary['correct_first_actions'],summary['successes']) or (valid>=12 and correct>=10)!=summary['gate']['passed'] or summary['automatic_retries'] or summary['optimizer_updates'] or summary['checkpoint_promotion']:raise ValueError('gate accounting')
 passed=bool(valid>=12 and correct>=10)
 report={'schema':'emender-e97-pi-native-stage-b-audit-v1','status':('qualified-gate-passed-not-promoted' if passed else 'qualified-negative-gate-failure-corrected-protocol' if corrected else 'qualified-negative-gate-failure'),'panel_sha256':plan['panel_sha256'],'plan_sha256':sha(a.plan),'checkpoint_sha256':plan['checkpoint_sha256'],'corrected_protocol':corrected,'episodes':14,'valid_first_frames':valid,'valid_first_frame_required':12,'correct_first_actions':correct,'correct_first_action_required':10,'successes_as_frozen':successes,'gate_passed':passed,'episode_receipts':receipts,'panel_expected_final_inconsistencies_not_regraded':['choice-read-explicit expects done although prompt requests facts.txt content','choice-grep-symbol expects done although prompt requests relative path'],'automatic_retries':0,'optimizer_updates':0,'checkpoint_promotion':False,'promotion_blocked':True if not passed else False,'promotion_authorized':False,'diagnostic_evaluation_allowed':True,'further_optimizer_updates_authorized':False,'checker_sha256':sha(__file__)}
