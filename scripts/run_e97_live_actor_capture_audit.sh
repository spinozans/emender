...
CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" scripts/audit_e97_live_actor_capture.py freeze --output "$RUN"
...
timeout --kill-after=30 4200 "$PYTHON_BIN" -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node=2 --max-restarts=0 \
 scripts/numa_local_rank_exec.py scripts/audit_e97_live_actor_capture.py run \
 --recipe "$RUN/recipe-private.json" --recipe-sha "$RECIPE_SHA" --output "$RUN" 2>&1 | tee "$RUN/console.log"
CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" scripts/audit_e97_live_actor_capture.py aggregate \
