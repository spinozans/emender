...
timeout --kill-after=30 900 "$PYTHON_BIN" -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node=1 --max-restarts=0 \
 scripts/numa_local_rank_exec.py scripts/diagnose_e97_early_prompt.py run --output "$RUN" --plan-sha "$PLAN_SHA" 2>&1 | tee "$RUN/worker.log"
