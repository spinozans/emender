#!/usr/bin/env bash
# Matched-compute LM run for the complex-eigenvalue head (task complex-eig-lm).
# Usage: run_ceig_lm.sh <gpu> <level> <mlp_ratio> <budget_flag> <budget_val> <outdir> [layer_kwargs_json]
...
cd /home/erikg/ndm/.wg-worktrees/agent-1284
...
echo "[run_ceig] GPU=$GPU level=$LEVEL mlp=$MLP $BFLAG=$BVAL out=$OUT lk=$LK"
