#!/usr/bin/env bash
set -euo pipefail
if [[ ${E97_EXECUTION_BOUNDED:-0} != 1 ]]; then
  export E97_EXECUTION_BOUNDED=1
...
cd "$CONTROL"
CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" scripts/eval_e97_native_execution.py freeze \
 --training "$R/native-training-50m-agent-lr1e5-v1" --runtime "$R" --output "$OUT"
PANEL_SHA=$(sha256sum "$OUT/panel.json" | cut -d' ' -f1)
mkdir -m 700 "$OUT/preflight" "$OUT/results"
...
timeout --kill-after=15 900 env CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" scripts/eval_e97_native_execution.py smoke \
 --panel "$OUT/panel.json" --panel-sha "$PANEL_SHA" --output "$OUT/preflight" | tee "$OUT/preflight.log"
...
"$PYTHON_BIN" -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node=8 --max-restarts=0 \
 scripts/numa_local_rank_exec.py scripts/eval_e97_native_execution.py run --panel "$OUT/panel.json" \
 --panel-sha "$PANEL_SHA" --output "$OUT/results" 2>&1 | tee "$OUT/execution.log"
CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" scripts/eval_e97_native_execution.py aggregate \
