...
"$PYTHON_BIN" -m pytest -p no:cacheprovider -q tests/test_e97_grounded_curriculum.py tests/test_e97_native_onpolicy_canary.py tests/test_e97_grounding_correction.py tests/test_e97_open_swe_native_runtime_protocol.py tests/test_e97_open_swe_native_sft.py
sha256sum -c "$FROZEN_SOURCE_INVENTORY" >/dev/null
timeout --kill-after=30 1800 "$PYTHON_BIN" -m scripts.build_e97_grounded_expansion --recipe configs/pi/e97-grounded-expansion-v1.json --output "$OUT"
