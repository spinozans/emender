#!/usr/bin/env bash
# Uniform FP32 workspace repair; no updates or replaced historical probabilities.
set -euo pipefail
umask 077
...
CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" -m pytest -p no:cacheprovider -q tests/test_e97_numerical_policy.py tests/test_e97_recurrent_precision.py tests/test_e97_fp32_state_layer_cuda.py tests/test_e97_facade.py tests/test_e97_sft_precision_integration.py tests/test_e97_fixed_recurrent_kernel_audit.py tests/test_e97_outcome_rl_candidate.py tests/test_e97_recurrent_operands.py tests/test_e97_first_divergence.py > "$RUN/cpu-tests.log" 2>&1
CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" -m scripts.qualify_e97_uniform_workspace freeze --output "$RUN"
...
assert len(cases)==14 and all(not any(c.find(t) is not None for t in ('skipped','failure','error')) for c in cases), 'fourteen executed CUDA cases required'
PY
for i in 1 2; do
...
CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" -m scripts.qualify_e97_uniform_workspace audit --output "$RUN"
