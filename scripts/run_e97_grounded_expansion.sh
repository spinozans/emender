#!/usr/bin/env bash
# Separate attended preparation/training/evaluation; never reopen a completed phase.
set -euo pipefail
umask 077
MODE=${1:?prepare, train, or evaluate required}
DATA=${2:?verified data root required}
RUN=${3:?new training phase root required}
...
case "$MODE" in
 prepare)
  ...
  CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" -m pytest -p no:cacheprovider -q tests/test_e97_grounded_curriculum.py tests/test_e97_grounded_expansion_gate.py tests/test_e97_grounding_correction.py tests/test_e97_native_training_segment.py tests/test_e97_native_execution.py
  CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" -m scripts.audit_e97_grounded_expansion --data "$DATA"
  ...
  CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" -m scripts.prepare_e97_grounded_expansion exposure --data "$DATA"
  CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" -m scripts.prepare_e97_grounded_expansion prepare --recipe configs/pi/e97-grounded-expansion-v1.json --data "$DATA" --phase "$RUN"
  echo GROUNDED_EXPANSION_READY_FOR_TRAINING
  ;;
 train)
 ...
 evaluate)
  [[ ! -e "$RUN/evaluation" ]]
  CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" -m scripts.prepare_e97_grounded_expansion evaluations --data "$DATA" --phase "$RUN"
  export GPU_LEASE_DIR=/home/erikg/emender/.wg/gpu_leases
  LEASE=$(bash scripts/gpu_lease.sh acquire 8 --no-wait)
  eval "$LEASE"
  # Compose cleanup with the lease helper instead of losing either EXIT handler.
  trap finish EXIT
  trap 'exit 130' INT
  trap 'exit 143' TERM
  ...
  CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" -m scripts.prepare_e97_grounded_expansion gate --phase "$RUN"
  ;;
