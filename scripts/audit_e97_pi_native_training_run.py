...
def audit(args):
 ...
 if recipe['operator_authorization_statement']!='I authorize the exact 32 update proposal.' or recipe['proposal_sha256']!='96db2e1cee91304b6c295113e517c8c1fd032c40a7fb372d9866dea8ada057be' or recipe['steps']!=32 or recipe['lr']!=1e-5 or recipe['automatic_retry'] or recipe['automatic_expansion'] or recipe['checkpoint_promotion'] or recipe['new_rl_updates']:raise ValueError('authorization scope')
 if sha(args.admission)!=recipe['admission_sha256'] or admission['status']!='authorized-exact-proposal' or admission['authorized_updates']!=32 or sha(recipe['schedule']['path'])!=recipe['schedule']['sha256'] or schedule['status']!='authorized-exact-proposal' or len(schedule['steps'])!=32:raise ValueError('admission/schedule identity')
 ...
 if len(starts)!=1 or len(steps)!=32 or len(checkpoints)!=1 or len(completes)!=1:raise ValueError('event coverage')
 start=starts[0]
 ...
 if checkpoint_path.parent!=(run/'checkpoints').resolve() or checkpoint_event['update']!=32 or sha(checkpoint_path)!=checkpoint_event['checkpoint_sha256'] or checkpoint_path.stat().st_size!=checkpoint_event['checkpoint_bytes']:raise ValueError('checkpoint publication')
 complete=completes[0]
 if complete['updates']!=32 or complete['requested_steps']!=32 or complete['reason'] is not None or complete['total_tokens']!=total_tokens or complete['assistant_target_tokens']!=total_targets:raise ValueError('completion receipt')
 checkpoint=torch.load(checkpoint_path,map_location='cpu',mmap=True,weights_only=False)
 expected={'sft_updates':32,'sft_total_tokens':total_tokens,'assistant_target_tokens':total_targets,'sampler_cursor':32,'sampler_key':recipe['schedule']['sampler_key'],'learning_rate':1e-5,'data_world_size':8,'diloco_k':4,'diloco_merge_enabled':False,'parent_checkpoint_sha256':recipe['parent_sha256'],'authority_manifest_sha256':recipe['authority_manifest_sha256'],'pack_manifest_sha256':recipe['pack_manifest_sha256'],'source_commit':recipe['source_commit']}
 ...
 if group['k']!=32 or group['train_mode'] is not False or ids!=set(optimizer['state']) or ids!=set(optimizer['eval_live_y']):raise ValueError('optimizer state coverage')
 ...
 report={'schema':'emender-e97-pi-native-curriculum-training-audit-v1','status':'passed-training-not-promoted','actual_training':True,'proposal_sha256':recipe['proposal_sha256'],'admission_sha256':recipe['admission_sha256'],'recipe_sha256':sha(run/'recipe.json'),'schedule_sha256':recipe['schedule']['sha256'],'updates':32,'input_tokens':total_tokens,'assistant_target_tokens':total_targets,'source_target_totals':dict(source),'runtime_sample_ids_and_clocks_exact':True,'step_losses':losses,'final_loss':checkpoint_event['loss'],'peak_rank0_hbm_allocated':max(x['max_hbm_allocated'] for x in steps),'checkpoint':{'path':str(checkpoint_path),'sha256':checkpoint_event['checkpoint_sha256'],'bytes':checkpoint_event['checkpoint_bytes'],'finite_complete_bf16_state':True},'numerical_fresh_continuation':'not measured; exact restoration required; no tolerance fallback','automatic_retry':False,'automatic_expansion':False,'checkpoint_promotion':False,'stage_b_executed':False,'new_rl_updates':0,'checker_sha256':sha(__file__)}
