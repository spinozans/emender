#!/usr/bin/env python3
"""Build simplified RS-free dense-agent trajectories with typed finalization."""
...
def trace(kind: str, index: int, rng: random.Random):
    ...
    if kind == "lookup":
        ...
        tool = "lookup_owner" if field == "owner" else "lookup_budget"
        return f"What is the {field} of {project}?", [
            action(tool, {"project": project}),
...
def main() -> None:
    ...
    with paths["tokens"].open("wb") as token_file, paths["mask"].open("wb") as mask_file, paths["index"].open("wb") as index_file, paths["metadata"].open("w") as metadata_file:
        for index in range(args.records):
            ...
            identity = f"agent-v3-{kind}-{index:08d}"
            ...
            metadata_file.write(json.dumps({"id": identity, "source": f"emender-agent-{kind}-v3", "split": validation, "tokens": len(tokens), "targets": sum(masks)}, sort_keys=True) + "\n")
            offset += len(tokens)
            counts["records"] += 1
    ...
    manifest = {
        ...
        "purpose": "dense-e97-bounded-grounded-agent-v3-simplified-lookup",
