...
def web_case(i,port):
 ...
 if kind==0:
  ...
  claim='Python was first released in 1991';steps=[action('source_check',{'claim':claim,'queries':['Python first release 1991 official history'],'numResults':3,'fetchContent':True}),{'name':'finish','dynamic':'observation'}];prompt='Check the supplied historical claim against web sources, then summarize only the returned evidence.';family='source-check';
 elif kind in (2,3,4,5,6,7):
  steps=[action('fetch_content',{'url':url,'mode':'readable'}),finish(fact)];prompt=f'Fetch {url}, find the exact page-local phrase {fact!r} in the returned content, and finish with exactly that phrase.';family='fetch-content'
 elif kind in (8,9):
  steps=[action('fetch_content',{'url':url,'mode':'readable'}),{'name':'get_search_content','dynamic':'response-id'},finish(fact)];prompt=f'Fetch {url}, retrieve its stored content using the returned responseId, verify the phrase {fact!r}, and finish with exactly that phrase.';family='fetch-retrieve'
 elif kind in (10,11,12):
  good=url;bad=f'https://example.invalid/{tag}.html';steps=[action('fetch_content',{'url':bad,'mode':'readable'}),action('fetch_content',{'url':good,'mode':'readable'}),finish(fact)];prompt=f'Observe the failure from {bad}, then fetch {good}, verify {fact!r} in its returned content, and finish with exactly that phrase.';family='fetch-error-recovery';prefix=1
 else:
...
def collect(args):
 ...
 for i,case in enumerate(plan['cases']):
  row=execute_case(case,panel,enc,args.output/case['id'],args.pi_bin,manifest_path);rows.append(row)
  if (i+1)%20==0:print('VERIFIED_PI_NATIVE_CANDIDATES',i+1,flush=True)
