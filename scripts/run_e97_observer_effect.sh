...
CUDA_VISIBLE_DEVICES='' "$PYTHON_BIN" scripts/diagnose_e97_observer_effect.py freeze --output "$RUN"
...
timeout --kill-after=30 3600 "$PYTHON_BIN" -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node=1 --max-restarts=0 \
 scripts/numa_local_rank_exec.py scripts/diagnose_e97_observer_effect.py run \
