...
def freeze(a):
 ...
 checkpoint_sha=a.checkpoint_sha256 or CHECKPOINT_SHA
...
def grade(case,bridge,acts,before,after):
 first=acts[0] if acts else None;name=first and first['name'];arguments=first and first['arguments'];results=[m for m in bridge.history if m['role']=='toolResult'];tool_results=len(results);exact=bridge.final==case['expected_final'] if case['expected_final'] is not None else isinstance(bridge.final,str) and bool(bridge.final.strip());arguments_exact='expected_first_arguments' not in case or arguments==case['expected_first_arguments'];workspace=True
 ...
 tool_evidence=(name=='finish' or (tool_results>0 and all(not r.get('isError',False) and not r['content'][0]['text'].lower().startswith(('error:','validation failed')) for r in results)))
...
def run(a):
 ...
 for path,key in ((plan['panel'],'panel_sha256'),(plan['checkpoint'],'checkpoint_sha256'),(plan['parent_model_source_panel'],'parent_model_source_panel_sha256'),(plan['args_json'],'args_sha256'),(plan['pi_inventory'],'pi_inventory_sha256'),(plan['tool_manifest'],'tool_manifest_sha256'),(plan['stage_b_tools'],'stage_b_tools_sha256'),(plan['cli_image'],'cli_image_sha256')):
...
def main():
 os.umask(0o077);signal.signal(signal.SIGTERM,lambda s,f:(_ for _ in ()).throw(TimeoutError('interrupted')));p=argparse.ArgumentParser();s=p.add_subparsers(dest='command',required=True);f=s.add_parser('freeze');f.add_argument('--panel',type=Path,required=True);f.add_argument('--checkpoint',type=Path,required=True);f.add_argument('--checkpoint-sha256',default=None);f.add_argument('--pi-bin',type=Path,required=True);f.add_argument('--pi-inventory',type=Path,required=True);f.add_argument('--cli-image',type=Path,required=True);f.add_argument('--cli-image-sha256',required=True);f.add_argument('--output',type=Path,required=True);r=s.add_parser('run');r.add_argument('--plan',type=Path,required=True);r.add_argument('--plan-sha',required=True);r.add_argument('--output',type=Path,required=True);a=p.parse_args();freeze(a) if a.command=='freeze' else run(a)
