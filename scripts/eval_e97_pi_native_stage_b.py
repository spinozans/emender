...
def main():
 os.umask(0o077);signal.signal(signal.SIGTERM,lambda s,f:(_ for _ in ()).throw(TimeoutError('interrupted')));p=argparse.ArgumentParser();s=p.add_subparsers(dest='command',required=True);f=s.add_parser('freeze');f.add_argument('--panel',type=Path,required=True);f.add_argument('--checkpoint',type=Path,required=True);f.add_argument('--pi-bin',type=Path,required=True);f.add_argument('--pi-inventory',type=Path,required=True);f.add_argument('--cli-image',type=Path,required=True);f.add_argument('--cli-image-sha256',required=True);f.add_argument('--output',type=Path,required=True);r=s.add_parser('run');r.add_argument('--plan',type=Path,required=True);r.add_argument('--plan-sha',required=True);r.add_argument('--output',type=Path,required=True);a=p.parse_args();freeze(a) if a.command=='freeze' else run(a)
