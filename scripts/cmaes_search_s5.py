#!/usr/bin/env python3
"""
CMA-ES v2 (S5-SYMMETRIC variant): Architecture/Hyperparameter Search.

This is a REPO COPY of scripts/cmaes_search_v2.py extended with a second,
selectable fitness objective for the S5-symmetric protocol
(paper/review/S5_SYMMETRIC_PROTOCOL.md, §D).

  --objective lm_loss      (DEFAULT) — the original §5 language-modeling-loss
                           path is byte-for-byte unchanged. Every function the
                           LM path uses (parse_average_loss, build_train_command,
                           run_training, estimate_params_for_config, ...) behaves
                           exactly as in cmaes_search_v2.py.
  --objective s5_acc@T128  — each CMA candidate runs the REAL state-tracking
                           harness experiments/expressivity_tasks/train_hybrid.py
                           --task s5_permutation (seq_len 128) and the fitness is
                           1 - mean_S5_acc@T128 averaged over the final
                           S5_WINDOW_STEPS training steps; NaN/divergence -> 1.0.
                           No synthetic fitness, no mock eval: the accuracy is the
                           eval_acc field train_hybrid already writes to its JSON.

The S5 path NEVER calls parse_average_loss / train.py. It is wired through
build_s5_train_command + parse_s5_fitness + run_training_s5 and gates candidates
on the REAL constructed parameter count (the analytic estimator
estimate_params_for_config is miscalibrated for the 8M hybrid GDN layer:
it predicts 6.7M/13.3M where the real model is 8.33M, so the S5 path counts
parameters by constructing the exact HybridLadderLM that will be trained).

Usage (LM, unchanged):
    python cmaes_search_v2.py --model e88 --train_minutes 30 --gpus 0,1,2,3,4,5,6,7

Usage (S5-symmetric, per protocol §D — see paper/review/S5_SYMMETRIC_LAUNCH.md):
    CUDA_VISIBLE_DEVICES=2,3,4,5 python scripts/cmaes_search_s5.py \
        --model e88 --objective s5_acc@T128 --phase cmaes \
        --anchor_configs .../seeds_s5_symmetric.json --anchor_only_cmaes \
        --params 8M --param_tolerance 0.10 \
        --popsize 16 --sigma 0.35 --min_generations 3 --converge 0.005 \
        --consecutive 3 --max_generations 12 --gpus 2,3,4,5 \
        --output experiments/expressivity_tasks/results/s5_symmetric_20260603
"""
...
# =============================================================================
# S5-SYMMETRIC OBJECTIVE (protocol §D) — set from --objective in main()
# =============================================================================
# OBJECTIVE selects the CMA-ES fitness. 'lm_loss' is the original §5 path and is
# the default; 's5_acc@T128' routes every candidate through the REAL
# state-tracking harness (train_hybrid.py --task s5_permutation).
OBJECTIVE = 'lm_loss'
OBJECTIVES = ('lm_loss', 's5_acc@T128')

# Per-candidate budget for the S5 objective (protocol §D.3). Identical for all
# arms. These are FIXED (not searched): only dim/depth/n_heads/n_state/lr and
# — for E88 only — linear_state/use_gate are searched (see S5_SEARCH_SPACES).
S5_TASK = 's5_permutation'
S5_STEPS = 5000            # per-candidate cap (full 20k reserved for winners)
S5_SEQ_LEN = 128           # train length T (selection length)
S5_BATCH = 32
...
# =============================================================================
# S5-SYMMETRIC SEARCH SPACES (protocol §D.2) — used only when OBJECTIVE is s5.
# =============================================================================
# Seeded from each arm's 8M MODEL_CONFIG center (run_separation_suite.py:77-112):
#   E88_8M {dim384,H32,N32}  M2RNN_8M {dim384,H32,N32}
#   FLA_8M {dim640,H32,N32}  M2RNN_paper_8M {dim608,H32,N32}   depth=4 all.
# VARY (all arms): dim (with ~8M real-param re-match, PARAM_TOLERANCE 0.10),
#   depth 3-6, n_heads, n_state {16,32}, lr (log).
# VARY (E88 ONLY): linear_state {0,1} and use_gate {0,1} — the BL-1 component,
#   now searched on S5 symmetrically rather than hand-kept on a proxy.
# FIX (all arms): update-rule family, optimizer (schedule-free AdamW),
#   train length T=128, eval grid {128,256,512,1024}, ~8M param target.
#
# dim is encoded int_mult16 over a band that brackets each seed; the ~8M
# real-param gate (is_valid_param_count -> real_param_count_s5) is what actually
# holds every candidate at 8M ±10%, so dim/depth/n_heads co-vary under that
# joint constraint. n_state is restricted to {16,32} via the e88_n_state snap.
_S5_COMMON_SPACE = {
    'dim':    (256, 768, 'int_mult16', 'Model dimension (real-param re-matched to ~8M)'),
    'depth':  (3, 6, 'int', 'Number of layers'),
    'n_heads':(8, 64, 'int', 'Number of heads'),
    'n_state':(16, 32, 'e88_n_state', 'State dimension {16,32}'),
...
def get_search_space(model_type, fixed_params=None):
    ...
    # S5-symmetric objective uses the 8M probe search spaces (protocol §D.2),
    # NOT the §5 LM-scale (dim 1024-4096, depth 10-50) spaces. The LM path
    # (OBJECTIVE == 'lm_loss') is unaffected and falls through unchanged.
    if OBJECTIVE == 's5_acc@T128':
        if model_type not in S5_SEARCH_SPACES:
            raise ValueError(
                f"--objective s5_acc@T128 supports models "
                f"{sorted(S5_SEARCH_SPACES)}, got '{model_type}'")
        return dict(S5_SEARCH_SPACES[model_type])
...
# --- S5 real parameter accounting -------------------------------------------
# The analytic estimate_params_for_config is calibrated for the §5 LM models at
# large scale and MISPREDICTS the 8M hybrid GDN layer (6.7M / 13.3M vs a real
# 8.33M). For the S5 objective we therefore count parameters by CONSTRUCTING the
# exact HybridLadderLM that train_hybrid will train (CPU, no GPU, no training).
_S5_TASK_VOCAB = None
_S5_PARAM_CACHE = {}


def _s5_task_vocab():
    global _S5_TASK_VOCAB
    if _S5_TASK_VOCAB is None:
        from experiments.expressivity_tasks.tasks import ALL_TASKS
        _S5_TASK_VOCAB = int(ALL_TASKS[S5_TASK]().vocab_size)
    return _S5_TASK_VOCAB


def s5_layer_kwargs_for(model_type, params):
    """Return (layer_level, per_layer_kwargs) for a candidate. For E88 the
    searched BL-1 knobs (linear_state, use_gate) become per-layer kwargs that
    HybridLadderLM forwards to the E88 layer; other arms use empty kwargs."""
    level = S5_LAYER_LEVEL[model_type]
    kw = {}
    if model_type == 'e88':
        if 'linear_state' in params:
            kw['linear_state'] = bool(int(params['linear_state']))
        if 'use_gate' in params:
            kw['use_gate'] = bool(int(params['use_gate']))
    return level, kw


def real_param_count_s5(params, model_type):
    """Exact param count of the HybridLadderLM train_hybrid will build for this
    candidate. Constructed on CPU. Returns None if the config cannot be built
    (invalid head/dim combo, etc.) — such candidates are rejected by the gate."""
    level, kw = s5_layer_kwargs_for(model_type, params)
    key = (model_type, int(params['dim']), int(params['depth']),
           int(params['n_heads']), int(params['n_state']),
           kw.get('linear_state'), kw.get('use_gate'))
    if key in _S5_PARAM_CACHE:
        return _S5_PARAM_CACHE[key]
    try:
        from ndm.models.hybrid_ladder import HybridLadderLM
        model = HybridLadderLM(
            vocab_size=_s5_task_vocab(),
            dim=int(params['dim']), depth=int(params['depth']),
            layer_pattern=[level], layer_kwargs=[kw] if kw else None,
            n_state=int(params['n_state']), n_heads=int(params['n_heads']),
            expansion=1.0,
        )
...
def is_valid_param_count(params, model_type, target_params, tolerance=None):
    ...
    if OBJECTIVE == 's5_acc@T128':
        # REAL constructed count (see real_param_count_s5); None => unbuildable.
        actual = real_param_count_s5(params, model_type)
        if actual is None:
            return False
...
# =============================================================================
# S5-SYMMETRIC TRAINING COMMAND + FITNESS (protocol §D)
# =============================================================================
S5_TIMEOUT_SECONDS = int(os.environ.get('S5_TIMEOUT_SECONDS', '14400'))  # 4h slack


def build_s5_train_command(params, model_type, output_dir, eval_id, seed=None):
    """Build the REAL train_hybrid.py command for one S5 candidate (protocol §D).

    Mirrors run_separation_suite.build_command for the 8M probe arms, but with the
    CMA-searched dim/depth/n_heads/n_state/lr (and, for E88 only, the searched
    linear_state/use_gate BL-1 knobs). No mock: train_hybrid runs the real
    s5_permutation task and writes eval_acc to <label>.json."""
    if seed is None:
        seed = S5_SEED
    level, _kw = s5_layer_kwargs_for(model_type, params)
    label = f's5cand_{model_type}_eval{eval_id}'
    cmd = [
        sys.executable,
        str(REPO_ROOT / 'experiments' / 'expressivity_tasks' / 'train_hybrid.py'),
        '--task', S5_TASK,
        '--layer_pattern', level,
        ...
        '--optimizer', S5_OPTIMIZER,
        '--steps', str(S5_STEPS),
        '--seq_len', str(S5_SEQ_LEN),
        '--batch_size', str(S5_BATCH),
        '--K', '5',  # s5_permutation ignores --K (built with no kwargs); mirrors suite
        '--seed', str(seed),
        '--label', label,
        '--output_dir', output_dir,
        '--eval_lengths', *[str(t) for t in S5_EVAL_LENGTHS],
        '--eval_lengths_n_batches', '8',
    ]
    # E88 BL-1 structural knobs (searched on S5): forward as real train_hybrid
    # flags. These default to None in train_hybrid (no behavior change for the
    # other arms / for the §6 probe), so passing them is purely additive.
    if model_type == 'e88':
        if 'linear_state' in params:
            cmd += ['--linear_state', str(int(params['linear_state']))]
        if 'use_gate' in params:
            cmd += ['--use_gate', str(int(params['use_gate']))]
    return cmd, label


def parse_s5_fitness(json_path, window_steps=None):
    """CMA fitness for the S5 objective (protocol §D.1):

        fitness = 1 - mean_S5_acc@T128

    where mean_S5_acc@T128 is the mean eval_acc (at the training length T=128)
    over the FINAL `window_steps` training steps of the capped run, read from the
    real eval_acc series train_hybrid writes. NaN / divergence / missing JSON ->
    fitness 1.0 (worst). Returns (fitness, mean_acc, n_window_points)."""
    import math
    if window_steps is None:
        window_steps = S5_WINDOW_STEPS
    try:
    ...
    # Explicit divergence guard on the final training loss.
    fl = log.get('final_loss')
    if fl is not None and (isinstance(fl, float) and (math.isnan(fl) or math.isinf(fl))):
        return 1.0, 0.0, 0
    ...
    if not steps:
        return 1.0, 0.0, 0
    try:
        max_step = max(int(s['step']) for s in steps)
    except (KeyError, ValueError, TypeError):
        return 1.0, 0.0, 0
    cutoff = max_step - int(window_steps)

    accs = []
    for s in steps:
        try:
            if int(s['step']) < cutoff:
                continue
            a = float(s['eval_acc'])
        except (KeyError, ValueError, TypeError):
            continue
        if math.isnan(a) or math.isinf(a):
            return 1.0, 0.0, len(accs)  # divergence in the selection window
        accs.append(a)

    if not accs:
        return 1.0, 0.0, 0
    mean_acc = sum(accs) / len(accs)
    if math.isnan(mean_acc) or math.isinf(mean_acc):
        return 1.0, 0.0, len(accs)
    return 1.0 - mean_acc, mean_acc, len(accs)
...
def run_training_s5(gpu_id, params, model_type, output_dir, eval_id):
    """Run ONE S5 candidate through the REAL train_hybrid harness (protocol §D).

    No batch-size memory probe (train_hybrid uses a fixed batch=32 at seq128),
    no LM loss. The returned 'loss' IS the CMA fitness = 1 - mean_S5_acc@T128.
    Every candidate's params.json, stdout/stderr, the train_hybrid <label>.json,
    and the .done summary are written under output_dir/eval_<id>/ so the records
    are committed (closes the A.2 NOT-FOUND gap)."""
    eval_dir = os.path.join(output_dir, f'eval_{eval_id}')
    os.makedirs(eval_dir, exist_ok=True)

    with open(os.path.join(eval_dir, 'params.json'), 'w') as f:
        json.dump({'params': params, 'model_type': model_type,
                   'eval_id': eval_id, 'objective': OBJECTIVE}, f, default=str)

    actual_params = real_param_count_s5(params, model_type)
    cmd, label = build_s5_train_command(params, model_type, eval_dir, eval_id)

    env = prepare_worker_env(model_type, gpu_id)
    cwd = str(REPO_ROOT)

    with open(os.path.join(eval_dir, 'cmd.txt'), 'w') as f:
        f.write(' '.join(cmd) + '\n')

    result = None
    try:
        result = subprocess.run(cmd, capture_output=True, text=True,
                                env=env, cwd=cwd, timeout=S5_TIMEOUT_SECONDS)
    except subprocess.TimeoutExpired:
        result = None
    except Exception as e:
        with open(os.path.join(eval_dir, 'run_error.txt'), 'w') as f:
    ...
    json_path = os.path.join(eval_dir, f'{label}.json')
    fitness, mean_acc, n_pts = parse_s5_fitness(json_path)

    result_dict = {
        'params': params,
        'actual_params': actual_params,
        'loss': fitness,             # CMA fitness = 1 - mean_S5_acc@T128
        's5_acc_at_T128': mean_acc,
        's5_window_points': n_pts,
        'final_loss': fitness,       # reporting mirror (keeps downstream code happy)
        'batch_size': S5_BATCH,
        'target_batch_size': S5_BATCH,
        'eval_id': eval_id,
        'gpu_id': gpu_id,
...
def run_training(gpu_id, params, model_type, train_minutes, output_dir, eval_id):
    ...
    if OBJECTIVE == 's5_acc@T128':
        # State-tracking objective: real train_hybrid eval, no LM loss, no probe.
        return run_training_s5(gpu_id, params, model_type, output_dir, eval_id)
    if PROGRESSIVE:
...
def run_cmaes_phase(model_type, train_minutes, output_dir, gpus,
    ...
    for ws_idx, warm_start in enumerate(warm_starts):
        ...
        if resume_state is not None and ws_idx == resume_state['ws_idx']:
            ...
            # LM path: smaller sigma for refinement (sigma0 is for exploration).
            # S5-symmetric path: protocol §D.3 fixes sigma=0.35 for the from-seed
            # CMA of every arm, so use sigma0 directly (no 0.4 shrink) for fairness.
            if OBJECTIVE == 's5_acc@T128':
                refinement_sigma = sigma0  # 0.35, identical for all arms
            else:
...
def main():
    ...
    parser.add_argument('--max_generations', type=int, default=30,
                        help='Hard generation cap per warm start (S5 protocol §D.3 uses 12)')
    ...
    parser.add_argument('--objective', type=str, default='lm_loss',
                        choices=list(OBJECTIVES),
                        help="CMA-ES fitness objective. 'lm_loss' (default) is the "
                             "unchanged §5 LM-loss path; 's5_acc@T128' runs the real "
                             "train_hybrid s5_permutation eval and minimizes "
    ...
    if OBJECTIVE == 's5_acc@T128' and args.model not in S5_SEARCH_SPACES:
        parser.error(
            ...
            f"{sorted(S5_SEARCH_SPACES)}, got '{args.model}'")
    ...
    print(f"Objective: {OBJECTIVE}"
          + (f"  (real harness: train_hybrid.py --task {S5_TASK}, "
             f"steps={S5_STEPS}, seq={S5_SEQ_LEN}, batch={S5_BATCH}, "
             f"fitness=1-mean_acc over final {S5_WINDOW_STEPS} steps)"
             if OBJECTIVE == 's5_acc@T128' else ''))
    if OBJECTIVE == 's5_acc@T128':
        print(f"Max generations (hard cap): {args.max_generations}")
    ...
    if args.phase == 'sweep':
        ...
        results = run_cmaes_phase(
            ...
            max_generations=args.max_generations, popsize=args.popsize
        )
        ...
        with open(results_file, 'w') as f:
            json.dump({
                ...
                'all_results': [{'params': r['params'],
                                 'loss': r['loss'],  # fitness (LM: avg loss; S5: 1-mean_acc)
                                 'final_loss': r.get('final_loss'),  # last-100 avg
                                 'actual_params': r.get('actual_params'),
                                 's5_acc_at_T128': r.get('s5_acc_at_T128'),
                                 's5_window_points': r.get('s5_window_points'),
                                 'candidate_json': r.get('candidate_json'),
