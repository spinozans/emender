...
def plan(args):
 ...
 if sha(args.authority/'manifest.json')!=args.authority_sha256 or sha(args.packs/'manifest.json')!=args.pack_sha256 or packs['sampler_mode']!='epoch-permutation' or packs['authority_manifest_sha256']!=args.authority_sha256:raise ValueError('planning authority identity')
 admitted=bool(authority['training_eligible'])
 if admitted:
  if args.admission is None:raise ValueError('training authority requires admission receipt')
  receipt=json.loads(args.admission.read_text())
  if receipt['status']!='authorized-exact-proposal' or receipt['authority_manifest_sha256']!=args.authority_sha256 or receipt['pack_manifest_sha256']!=args.pack_sha256 or receipt['authorized_updates']!=args.steps or not packs['training_eligible']:raise ValueError('training admission')
 elif args.admission is not None or packs['training_eligible']:raise ValueError('nontraining planning authority')
