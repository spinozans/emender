...
def audit(args):
 ...
 cohorts={'openhands-execution-rehearsal','pi-native-curriculum','representation-bridge-rehearsal','grounded-authored-rehearsal','grounded-correction-rehearsal'}
 ...
 if authority['correction_rehearsal'] is None or authority['correction_rehearsal']['authority_sha256']!=proposal['correction_source_sha256']:raise ValueError('correction slice binding')
 corr=json.loads((Path(proposal['correction_rehearsal_source'])/'manifest.json').read_text())
 if corr.get('schema')!='emender-e97-tulu3-masked-sft-v1' or corr.get('training_eligible') is not True:raise ValueError('correction source state')
 ...
 receipt={'schema':'emender-e97-pi-native-repair3-proposal-audit-v1','status':'qualified-proposal-not-authorized',
  ...
  'per_update_cohort_stratification':'verified: all 32 updates contain all five cohorts',
  ...
  'scheduled_correction_targets':proposal['scheduled_source_targets']['grounded-correction-rehearsal'],
