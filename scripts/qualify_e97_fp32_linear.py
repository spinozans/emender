...
def freeze(args):
    ...
    if sha(p)!=RECIPE_SHA:raise ValueError('candidate recipe identity')
    publish(args.output/'plan.json',dict(schema='e97-fp32-linear-qualification-v1',kernel=FIXED_RECURRENT_KERNEL,numerical_policy=POLICY,
        recipe_sha256=sha(p),base_recipe_sha256=BASE_SHA,workers=2,kernel_cases=13,kernel_seconds=900,
...
def audit(args):
    ...
    if plan['schema']!='e97-fp32-linear-qualification-v1' or plan['numerical_policy']!=POLICY or plan['recipe_sha256']!=RECIPE_SHA:raise ValueError('candidate plan')
    check(args.output,expected_recipe_sha=plan['recipe_sha256'],expected_kernel_cases=13,numerical_policy=POLICY)
