# Repo Conventions (Claude / agents)

> **AGENTS.md and CLAUDE.md are kept in lock-step.** Edit both, or edit one and
> copy it to the other. They contain the same operating rules.
...
- Lease files live at `.wg/gpu_leases/<gpu_id>` (`.wg` is a **shared** symlink,
  ...
  `pid`, `starttime` (PID-reuse guard), `host`, `created`, `heartbeat`.
