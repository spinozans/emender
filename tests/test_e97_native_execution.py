...
def test_episode_feeds_observation_before_finish(monkeypatch, tmp_path):
    import tiktoken
    ...
    def generate(loaded, prompt, encoding, budget, deadline):
        ...
        name, args = ('str_replace_editor', {'command': 'view', 'path': '/testbed/config.json'}) if len(prompts) == 1 else ('finish', {'message': c['answer']})
