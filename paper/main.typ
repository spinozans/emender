...
Triton multi-programming kernel are assigned to the v1 HuggingFace
release target; the trusted Lean 4 core has no
...
predictions; §12 future work. The v1 artifact plan assigns E88 to the
HuggingFace model repository `ekg/emender-e88-1.27b`, with companion
M²RNN-CMA and GDN baseline artifacts, the per-architecture CMA-ES
configurations, and the Triton multi-programming kernel in the same
release bundle.
...
*Release.* The v1 artifact plan assigns the E88 1.27 B checkpoint to
the HuggingFace model repository `ekg/emender-e88-1.27b`, with
companion M²RNN-CMA and Gated DeltaNet baseline artifacts. The same
release bundle includes the per-architecture CMA-ES configurations,
the training protocol, and the Triton multi-programming kernel source.
