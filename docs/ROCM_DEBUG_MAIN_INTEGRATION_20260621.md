...
- base/current `origin/main`: `2dee6ce` (`feat: rocm-gdn2-mlp-port (agent-28)`)
...
cherry -v HEAD <branch>` reported patch-equivalent commits with `-`, meaning
their exact branch tips have no remaining patch delta against the integrated
tree. The gdn2 branch contains original/split commits plus a merge commit, while
`main` contains the same file content as squashed commit `2dee6ce`; `git diff
HEAD wg/agent-28/rocm-gdn2-mlp-port` was empty.
...
The in-progress `rocm-kernel-integration-smoke` task had prepared useful smoke
runner/report material before this task confirmed the integrated main state. Its
report explicitly states that exact predecessor branch tips were not ancestors
of its then-current `HEAD` at smoke-preparation time and that its artifacts are
pre-merge blocker evidence only.

Useful generic pieces from that work were preserved here:
...
The smoke task should rerun or update evidence from integrated main after this
task completes. Its pre-merge logs should not admit any variant into the
benchmark matrix.
...
```bash
git cherry -v HEAD wg/agent-14/frontier-allocation-ledger
git cherry -v HEAD wg/agent-16/frontier-env-debug-recipe
git cherry -v HEAD wg/agent-23/rocm-kernel-port-audit
git cherry -v HEAD wg/agent-27/rocm-e97-mlp-port
git diff HEAD wg/agent-28/rocm-gdn2-mlp-port
