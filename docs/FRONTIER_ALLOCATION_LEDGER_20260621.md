...
| `DEBUG-POSTCACHE-20260621-E97` | `stage-p50k-cache` | `debug` | `failed` | `N/A` | `docs/FRONTIER_POST_CACHE_DEBUG_20260621.md` | `4881174`, `4881258` | debug | 2026-06-21 | 2026-06-21T14:42:14 | 2026-06-21T15:15:59 | 1 per job | 00:30:00 per job | 1.000000 | 0.274722 elapsed | 19,999.802776 | 4,928 | 19,999.528054 | e97-MLP fused split-edit Triton, post-cache | `post-cache-debug-e97` | `TIKTOKEN_CACHE_DIR=/lustre/orion/bif148/proj-shared/tiktoken_cache`; `p50k_base.tiktoken` readable from compute-node cache probe; smoke DATA `/lustre/orion/bif148/scratch/erikgarrison/emender/data/commapile_mainmix_smoke.txt`; runtime `EMENDER_CONDA_ENV=base` | Observed: tokenizer cache blocker removed and `Tokenizer: p50k_base, vocab_size=50281` printed from compute. Job `4881174` stopped on a `LadderLM.__init__` interface mismatch, fixed in this task. Job `4881258` passed e97 kernel smoke, reached first finite loss `11.1347`, last observed `step 50 loss 6.0547`, throughput about `3202 tok/s`, `PEAK_MEMORY_MB=13115`, fused/no-eager guard, and wrote `checkpoint_step_000050_loss_7.7509.pt`; it then failed on a rank-0 checkpoint symlink race that was fixed in the debug script after the run. | `docs/FRONTIER_POST_CACHE_DEBUG_20260621.md`; `/lustre/orion/bif148/proj-shared/emender/frontier_runs/debug/20260621/e97-MLP/4881258-20260621T150136Z`; `logs/frontier/debug/emender-smoke-4881258.out`; `logs/frontier/debug/emender-smoke-4881258.err`; `scripts/frontier/debug_smoke_one_node.slurm` | Rerun e97-MLP once with the fixed rank-mapping script for clean exit before any e97 canary; tokenizer no-go is resolved. |
| `DEBUG-POSTCACHE-20260621-GDN2` | `stage-p50k-cache` | `debug` | `completed` | `N/A` | `docs/FRONTIER_POST_CACHE_DEBUG_20260621.md` | `4881346`, `4881374` | debug | 2026-06-21 | 2026-06-21T15:17:09 | 2026-06-21T15:25:59 | 1 per job | 00:30:00 per job | 1.000000 | 0.128889 elapsed | 19,999.528054 | 4,928 | 19,999.399165 | gdn2-MLP external GDN2 fused path, post-cache | `post-cache-debug-gdn2` | `TIKTOKEN_CACHE_DIR=/lustre/orion/bif148/proj-shared/tiktoken_cache`; `p50k_base.tiktoken` readable from compute-node cache probe; GDN2 checkout `/lustre/orion/bif148/scratch/erikgarrison/emender/src/GatedDeltaNet-2`; smoke DATA `/lustre/orion/bif148/scratch/erikgarrison/emender/data/commapile_mainmix_smoke.txt`; runtime `EMENDER_CONDA_ENV=base` | Observed: job `4881346` verified cache and fused GDN2 guard but failed before first training loss because `LOCAL_RANK=$SLURM_LOCALID` was invalid with rank-local `--gpus-per-task=1` visibility; fixed in the debug script. Job `4881374` completed, reached first finite loss `11.2562`, final `step 262 loss 0.0363`, `global_tok/s 34870`, `PEAK_MEMORY_MB=16847`, fused/no-eager guard, and checkpoint artifact. | `docs/FRONTIER_POST_CACHE_DEBUG_20260621.md`; `/lustre/orion/bif148/proj-shared/emender/frontier_runs/debug/20260621/gdn2-MLP/4881374-20260621T152028Z`; `logs/frontier/debug/emender-smoke-4881374.out`; `logs/frontier/debug/emender-smoke-4881374.err`; `scripts/frontier/debug_smoke_one_node.slurm` | gdn2-MLP one-node post-cache debug gate passed; use as contrast evidence for scaleout planning, not as e97 extended approval. |
...
| `DILOCO-8x4H-CANARY` | `review-frontier-evidence` | `scaleout` | `candidate` | `REJECTED` | `docs/FRONTIER_DILOCO_8X4H_CANARY_REVIEW_20260621.md` | `TBD` | regular | `TBD` | `TBD` | `TBD` | 8 | 04:00:00 | 32 | `TBD` | 19,999.802776 | 4,928 | 19,967.802776 | ScheduleFree local-SGD/DiLoCo, `--diloco_island_size 8`, selected arm after post-cache debug evidence | `canary-k250-beta0` | ctx2k, one-node islands, commapile, shared tokenizer cache required | Not submitted. Rejected by `review-frontier-evidence` because post-cache debug evidence, selected-arm first finite loss/throughput, and checkpoint/resume artifacts are not yet present. Hypothesis remains: 8 one-node islands can expose launch/RCCL/merge behavior cheaply after one-node gates pass. | `docs/FRONTIER_DILOCO_8X4H_CANARY_REVIEW_20260621.md`; `docs/FRONTIER_DILOCO_SCALEOUT_READINESS_20260621.md`; `scripts/frontier/diloco_scaleout_readiness.sbatch` | Complete `stage-p50k-cache` and `verify-selected-frontier`, then create a new approval artifact before any `sbatch`. |
