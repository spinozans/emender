#!/usr/bin/env bash
# ============================================================================
# repair-v9r full-arc SEGMENT 4 of 8 — STAGED COMMAND SHEET (NOT EXECUTED).
#
...
#
# Segment sampler key 1400027 (screened order); segment schedule schedule-segment4-proposal.json.
# Juncture at u512 (Stage-B panel + conversation NLL probe; full frozen dual
# gates at u256/u512/u896/u1024 (THIS SEGMENT ENDS AT THE u512 FULL FROZEN DUAL GATE)); fail-closed stopping armed.
#
...
PROP=$REPO/configs/pi/e97-pi-native-repair9r-full-arc-segment4-training-proposal-v1.json
ADM=$P/pi-native-repair9r-full-arc-segment4-training-admission-v1
RUN=$P/pi-native-repair9r-full-arc-segment4-training-v1
...
PREV=$P/pi-native-repair9r-full-arc-segment3-training-v1
...
"$PY" scripts/audit_e97_pi_native_repair_generic.py --proposal "$PROP" \
  --schema emender-e97-pi-native-repair9r-full-arc-segment4-proposal-v1 \
  --schedule "$SCHED" \
  --output "$ST/proposal-audits/segment4-proposal-audit.json"

# ---- STEP 3: scoped commit + push of the frozen proposal
git add "$PROP"
git commit -m "Freeze repair-v9r full-arc segment 4 proposal"
...
bash "$ST/templates/instantiate-run-dir.sh" --segment 4 \
...
bash "$ST/templates/instantiate-juncture-dir.sh" --segment 4 --updates 512
# regenerate $P/pi-native-repair9r-full-arc-juncture-evals/seg4-u512/panel.json
# for the exact audited checkpoint, then:
# bash $P/pi-native-repair9r-full-arc-juncture-evals/seg4-u512/stage-juncture.sh
