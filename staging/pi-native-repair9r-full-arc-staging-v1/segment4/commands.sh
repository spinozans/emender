#!/usr/bin/env bash
# ============================================================================
# repair-v9r V3 RESTART arc — SEGMENT 4 of 8 — STAGED COMMAND SHEET (NOT EXECUTED).
#
...
# protocol unchanged.
#
# Segment sampler key 1401046 (screened order, fresh range 1401002..); segment
# schedule schedule-segment4-proposal.json. Juncture at u512 (Stage-B panel + conversation NLL probe;
# full frozen dual gates at u256/u512/u896/u1024 (THIS SEGMENT ENDS AT THE u512 FULL FROZEN DUAL GATE)); fail-closed
# stopping armed.
#
...
PROP=$REPO/configs/pi/e97-pi-native-repair9r-full-arc-v3-segment4-training-proposal-v1.json
ADM=$P/pi-native-repair9r-full-arc-v3-segment4-training-admission-v1
RUN=$P/pi-native-repair9r-full-arc-v3-segment4-training-v1
SCHED=$PREP/schedule-segment4-proposal.json
KEY=1401046
...
# ---- STEP 0: resolve the parent (fail-closed) segment 3's final audited checkpoint; the freeze-time writer and the generic auditor both verify the chained parent.
PREV=$P/pi-native-repair9r-full-arc-v3-segment3-training-v1
...
"$PY" "$ST/freeze_segment_proposal.py" --segment-index 4 \
...
"$PY" scripts/audit_e97_pi_native_repair_generic.py --proposal "$PROP" \
  --schema emender-e97-pi-native-repair9r-full-arc-v3-segment4-proposal-v1 \
  --schedule "$SCHED" \
  --output "$ST/proposal-audits/segment4-v3-proposal-audit.json"

# ---- STEP 3: scoped commit + push of the frozen proposal
git add "$PROP"
git commit -m "Freeze repair-v9r v3-restart arc segment 4 proposal (scrub-reversal prep)"
...
bash "$ST/templates/instantiate-run-dir.sh" --segment 4 --arc-suffix -v3 \
...
# ---- STEP 8: juncture evaluation at u512 (Stage-B panel + conversation NLL
...
#      cohort's first entry). Then run the FULL FROZEN DUAL GATE (uu512) before any continuation.
bash "$ST/templates/instantiate-juncture-dir.sh" --segment 4 --updates 512 --arc-suffix -v3
# regenerate $P/pi-native-repair9r-full-arc-v3-juncture-evals/seg4-u512/panel.json
# for the exact audited checkpoint, then:
# bash $P/pi-native-repair9r-full-arc-v3-juncture-evals/seg4-u512/stage-juncture.sh
...
#      If continuing: run segment 5 command sheet (chained to this
