#!/usr/bin/env bash
# ============================================================================
# repair-v9r V3 RESTART arc — SEGMENT 2 of 8 — STAGED COMMAND SHEET (NOT EXECUTED).
#
...
# protocol unchanged.
#
# Segment sampler key 1401042 (screened order, fresh range 1401002..); segment
# schedule schedule-segment2-proposal.json. Juncture at u256 (Stage-B panel + conversation NLL probe;
# full frozen dual gates at u256/u512/u896/u1024 (THIS SEGMENT ENDS AT THE u256 FULL FROZEN DUAL GATE)); fail-closed
# stopping armed.
#
...
PROP=$REPO/configs/pi/e97-pi-native-repair9r-full-arc-v3-segment2-training-proposal-v1.json
ADM=$P/pi-native-repair9r-full-arc-v3-segment2-training-admission-v1
RUN=$P/pi-native-repair9r-full-arc-v3-segment2-training-v1
SCHED=$PREP/schedule-segment2-proposal.json
KEY=1401042
...
# ---- STEP 0: resolve the parent (fail-closed) segment 1's final audited checkpoint; the freeze-time writer and the generic auditor both verify the chained parent.
PREV=$P/pi-native-repair9r-full-arc-v3-segment1-training-v1
...
"$PY" "$ST/freeze_segment_proposal.py" --segment-index 2 \
...
"$PY" scripts/audit_e97_pi_native_repair_generic.py --proposal "$PROP" \
  --schema emender-e97-pi-native-repair9r-full-arc-v3-segment2-proposal-v1 \
  --schedule "$SCHED" \
  --output "$ST/proposal-audits/segment2-v3-proposal-audit.json"

# ---- STEP 3: scoped commit + push of the frozen proposal
git add "$PROP"
git commit -m "Freeze repair-v9r v3-restart arc segment 2 proposal (scrub-reversal prep)"
...
bash "$ST/templates/instantiate-run-dir.sh" --segment 2 --arc-suffix -v3 \
...
# ---- STEP 8: juncture evaluation at u256 (Stage-B panel + conversation NLL
...
#      cohort's first entry). Then run the FULL FROZEN DUAL GATE (uu256) before any continuation.
bash "$ST/templates/instantiate-juncture-dir.sh" --segment 2 --updates 256 --arc-suffix -v3
# regenerate $P/pi-native-repair9r-full-arc-v3-juncture-evals/seg2-u256/panel.json
# for the exact audited checkpoint, then:
# bash $P/pi-native-repair9r-full-arc-v3-juncture-evals/seg2-u256/stage-juncture.sh
...
#      If continuing: run segment 3 command sheet (chained to this
