#!/usr/bin/env python3
"""Emit per-segment staged command sheets (segment1..8/commands.sh). STAGED; NOT EXECUTED."""
import argparse
from pathlib import Path

ST = Path('/mnt/nvme2n1/erikg/e97_systematic_posttraining/pi-native-repair9r-full-arc-staging-v1')
KEYS = [1400005, 1400010, 1400023, 1400027, 1400050, 1400057, 1400080, 1400081]

TEMPLATE = r'''#!/usr/bin/env bash
# ============================================================================
# repair-v9r full-arc SEGMENT {N} of 8 — STAGED COMMAND SHEET (NOT EXECUTED).
#
...
#
# Segment sampler key {KEY} (screened order); segment schedule {SCHED}.
# Juncture at u{U} (Stage-B panel + conversation NLL probe; full frozen dual
# gates at u256/u512/u896/u1024{GATE_LINE}); fail-closed stopping armed.
#
...
PROP=$REPO/configs/pi/e97-pi-native-repair9r-full-arc-segment{N}-training-proposal-v1.json
ADM=$P/pi-native-repair9r-full-arc-segment{N}-training-admission-v1
RUN=$P/pi-native-repair9r-full-arc-segment{N}-training-v1
SCHED=$PREP/{SCHED}
KEY={KEY}
...
"$PY" scripts/audit_e97_pi_native_repair_generic.py --proposal "$PROP" \
  --schema emender-e97-pi-native-repair9r-full-arc-segment{N}-proposal-v1 \
  --schedule "$SCHED" \
  --output "$ST/proposal-audits/segment{N}-proposal-audit.json"

# ---- STEP 3: scoped commit + push of the frozen proposal
git add "$PROP"
git commit -m "Freeze repair-v9r full-arc segment {N} proposal"
...
bash "$ST/templates/instantiate-run-dir.sh" --segment {N} \
...
bash "$ST/templates/instantiate-juncture-dir.sh" --segment {N} --updates {U}
# regenerate $P/pi-native-repair9r-full-arc-juncture-evals/seg{N}-u{U}/panel.json
# for the exact audited checkpoint, then:
# bash $P/pi-native-repair9r-full-arc-juncture-evals/seg{N}-u{U}/stage-juncture.sh
...
CHAINED_PARENT = '''PREV=$P/pi-native-repair9r-full-arc-segment__PREV__-training-v1
...
def main():
    ...
    for n in range(1, 9):
        ...
        if n == 1:
            parent_comment = 'bridge parent, full retention headroom.'
        ...
        text = TEMPLATE.format(N=n, KEY=KEYS[n - 1], SCHED=sched, U=u, GATE_LINE=gate_line,
                               PARENT_COMMENT=parent_comment, PARENT_BLOCK=parent_block,
