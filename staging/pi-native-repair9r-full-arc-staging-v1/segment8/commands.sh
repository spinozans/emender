#!/usr/bin/env bash
# ============================================================================
# repair-v9r V3 RESTART arc — SEGMENT 8 of 8 — STAGED COMMAND SHEET (NOT EXECUTED).
#
...
# protocol unchanged.
#
# Segment sampler key 1401074 (screened order, fresh range 1401002..); segment
# schedule schedule-segment8-proposal.json. Juncture at u1024 (Stage-B panel + conversation NLL probe;
# full frozen dual gates at u256/u512/u896/u1024 (THIS SEGMENT ENDS AT THE u1024 FULL FROZEN DUAL GATE)); fail-closed
# stopping armed.
#
...
PROP=$REPO/configs/pi/e97-pi-native-repair9r-full-arc-v3-segment8-training-proposal-v1.json
ADM=$P/pi-native-repair9r-full-arc-v3-segment8-training-admission-v1
RUN=$P/pi-native-repair9r-full-arc-v3-segment8-training-v1
...
PREV=$P/pi-native-repair9r-full-arc-v3-segment7-training-v1
...
"$PY" scripts/audit_e97_pi_native_repair_generic.py --proposal "$PROP" \
  --schema emender-e97-pi-native-repair9r-full-arc-v3-segment8-proposal-v1 \
  --schedule "$SCHED" \
  --output "$ST/proposal-audits/segment8-v3-proposal-audit.json"

# ---- STEP 3: scoped commit + push of the frozen proposal
git add "$PROP"
git commit -m "Freeze repair-v9r v3-restart arc segment 8 proposal (scrub-reversal prep)"
...
bash "$ST/templates/instantiate-run-dir.sh" --segment 8 --arc-suffix -v3 \
...
bash "$ST/templates/instantiate-juncture-dir.sh" --segment 8 --updates 1024 --arc-suffix -v3
# regenerate $P/pi-native-repair9r-full-arc-v3-juncture-evals/seg8-u1024/panel.json
# for the exact audited checkpoint, then:
# bash $P/pi-native-repair9r-full-arc-v3-juncture-evals/seg8-u1024/stage-juncture.sh
