#!/usr/bin/env bash
# ============================================================================
# repair-v9r full-arc SEGMENT 5 of 8 — STAGED COMMAND SHEET (NOT EXECUTED).
#
...
#
# Segment sampler key 1400050 (screened order); segment schedule schedule-segment5-proposal.json.
# Juncture at u640 (Stage-B panel + conversation NLL probe; full frozen dual
...
PROP=$REPO/configs/pi/e97-pi-native-repair9r-full-arc-segment5-training-proposal-v1.json
ADM=$P/pi-native-repair9r-full-arc-segment5-training-admission-v1
RUN=$P/pi-native-repair9r-full-arc-segment5-training-v1
...
PREV=$P/pi-native-repair9r-full-arc-segment4-training-v1
...
"$PY" scripts/audit_e97_pi_native_repair_generic.py --proposal "$PROP" \
  --schema emender-e97-pi-native-repair9r-full-arc-segment5-proposal-v1 \
  --schedule "$SCHED" \
  --output "$ST/proposal-audits/segment5-proposal-audit.json"

# ---- STEP 3: scoped commit + push of the frozen proposal
git add "$PROP"
git commit -m "Freeze repair-v9r full-arc segment 5 proposal"
...
bash "$ST/templates/instantiate-run-dir.sh" --segment 5 \
...
bash "$ST/templates/instantiate-juncture-dir.sh" --segment 5 --updates 640
# regenerate $P/pi-native-repair9r-full-arc-juncture-evals/seg5-u640/panel.json
# for the exact audited checkpoint, then:
# bash $P/pi-native-repair9r-full-arc-juncture-evals/seg5-u640/stage-juncture.sh
