#!/usr/bin/env bash
# ============================================================================
# repair-v9r full-arc SEGMENT 3 of 8 — STAGED COMMAND SHEET (NOT EXECUTED).
#
...
#
# Segment sampler key 1400023 (screened order); segment schedule schedule-segment3-proposal.json.
# Juncture at u384 (Stage-B panel + conversation NLL probe; full frozen dual
...
PROP=$REPO/configs/pi/e97-pi-native-repair9r-full-arc-segment3-training-proposal-v1.json
ADM=$P/pi-native-repair9r-full-arc-segment3-training-admission-v1
RUN=$P/pi-native-repair9r-full-arc-segment3-training-v1
...
PREV=$P/pi-native-repair9r-full-arc-segment2-training-v1
...
"$PY" scripts/audit_e97_pi_native_repair_generic.py --proposal "$PROP" \
  --schema emender-e97-pi-native-repair9r-full-arc-segment3-proposal-v1 \
  --schedule "$SCHED" \
  --output "$ST/proposal-audits/segment3-proposal-audit.json"

# ---- STEP 3: scoped commit + push of the frozen proposal
git add "$PROP"
git commit -m "Freeze repair-v9r full-arc segment 3 proposal"
...
bash "$ST/templates/instantiate-run-dir.sh" --segment 3 \
...
bash "$ST/templates/instantiate-juncture-dir.sh" --segment 3 --updates 384
# regenerate $P/pi-native-repair9r-full-arc-juncture-evals/seg3-u384/panel.json
# for the exact audited checkpoint, then:
# bash $P/pi-native-repair9r-full-arc-juncture-evals/seg3-u384/stage-juncture.sh
