#!/usr/bin/env bash
# ============================================================================
# repair-v9r full-arc SEGMENT 6 of 8 — STAGED COMMAND SHEET (NOT EXECUTED).
#
...
#
# Segment sampler key 1400057 (screened order); segment schedule schedule-segment6-proposal.json.
# Juncture at u768 (Stage-B panel + conversation NLL probe; full frozen dual
...
PROP=$REPO/configs/pi/e97-pi-native-repair9r-full-arc-segment6-training-proposal-v1.json
ADM=$P/pi-native-repair9r-full-arc-segment6-training-admission-v1
RUN=$P/pi-native-repair9r-full-arc-segment6-training-v1
...
PREV=$P/pi-native-repair9r-full-arc-segment5-training-v1
...
"$PY" scripts/audit_e97_pi_native_repair_generic.py --proposal "$PROP" \
  --schema emender-e97-pi-native-repair9r-full-arc-segment6-proposal-v1 \
  --schedule "$SCHED" \
  --output "$ST/proposal-audits/segment6-proposal-audit.json"

# ---- STEP 3: scoped commit + push of the frozen proposal
git add "$PROP"
git commit -m "Freeze repair-v9r full-arc segment 6 proposal"
...
bash "$ST/templates/instantiate-run-dir.sh" --segment 6 \
...
bash "$ST/templates/instantiate-juncture-dir.sh" --segment 6 --updates 768
# regenerate $P/pi-native-repair9r-full-arc-juncture-evals/seg6-u768/panel.json
# for the exact audited checkpoint, then:
# bash $P/pi-native-repair9r-full-arc-juncture-evals/seg6-u768/stage-juncture.sh
