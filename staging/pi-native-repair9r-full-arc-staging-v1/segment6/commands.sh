#!/usr/bin/env bash
# ============================================================================
# repair-v9r V3 RESTART arc — SEGMENT 6 of 8 — STAGED COMMAND SHEET (NOT EXECUTED).
#
...
# protocol unchanged.
#
# Segment sampler key 1401056 (screened order, fresh range 1401002..); segment
# schedule schedule-segment6-proposal.json. Juncture at u768 (Stage-B panel + conversation NLL probe;
...
PROP=$REPO/configs/pi/e97-pi-native-repair9r-full-arc-v3-segment6-training-proposal-v1.json
ADM=$P/pi-native-repair9r-full-arc-v3-segment6-training-admission-v1
RUN=$P/pi-native-repair9r-full-arc-v3-segment6-training-v1
SCHED=$PREP/schedule-segment6-proposal.json
KEY=1401056
...
# ---- STEP 0: resolve the parent (fail-closed) segment 5's final audited checkpoint; the freeze-time writer and the generic auditor both verify the chained parent.
PREV=$P/pi-native-repair9r-full-arc-v3-segment5-training-v1
...
"$PY" "$ST/freeze_segment_proposal.py" --segment-index 6 \
...
"$PY" scripts/audit_e97_pi_native_repair_generic.py --proposal "$PROP" \
  --schema emender-e97-pi-native-repair9r-full-arc-v3-segment6-proposal-v1 \
  --schedule "$SCHED" \
  --output "$ST/proposal-audits/segment6-v3-proposal-audit.json"

# ---- STEP 3: scoped commit + push of the frozen proposal
git add "$PROP"
git commit -m "Freeze repair-v9r v3-restart arc segment 6 proposal (scrub-reversal prep)"
...
bash "$ST/templates/instantiate-run-dir.sh" --segment 6 --arc-suffix -v3 \
...
# ---- STEP 8: juncture evaluation at u768 (Stage-B panel + conversation NLL
...
bash "$ST/templates/instantiate-juncture-dir.sh" --segment 6 --updates 768 --arc-suffix -v3
# regenerate $P/pi-native-repair9r-full-arc-v3-juncture-evals/seg6-u768/panel.json
# for the exact audited checkpoint, then:
# bash $P/pi-native-repair9r-full-arc-v3-juncture-evals/seg6-u768/stage-juncture.sh
...
#      If continuing: run segment 7 command sheet (chained to this
