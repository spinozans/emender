#!/usr/bin/env bash
# ============================================================================
# repair-v9r full-arc SEGMENT 1 of 8 — STAGED COMMAND SHEET (NOT EXECUTED).
#
...
#
# Segment sampler key 1400005 (screened order); segment schedule schedule-proposal.json.
# Juncture at u128 (Stage-B panel + conversation NLL probe; full frozen dual
# gates at u256/u512/u896/u1024); fail-closed stopping armed.
#
...
PROP=$REPO/configs/pi/e97-pi-native-repair9r-full-arc-segment1-training-proposal-v1.json
ADM=$P/pi-native-repair9r-full-arc-segment1-training-admission-v1
RUN=$P/pi-native-repair9r-full-arc-segment1-training-v1
...
# ---- STEP 0: resolve the parent (fail-closed) bridge parent, full retention headroom.
# STEPS 1-3 ARE ALREADY COMPLETE (T3, attempt 4):
#   frozen proposal  sha256 063b3f7567ce5306791f2ec3c745a7b732691ef442ee4eb577bc77bb0647e351
#   proposal audit   sha256 dc96fe255ef26b4b77a3dbeed5c4338037bc496dafa9bec4f6b6c4a2fa5e16bf
#                     (status qualified-proposal-not-authorized, receipt at
#                      $ST/proposal-audits/segment1-proposal-audit.json)
#   committed+pushed commit 2358816f "Freeze repair-v9r full-arc segment 1 proposal
#                     (translated+replay-verified OH restart)"
# Operator sign-off begins at STEP 4 (admission).
...
"$PY" scripts/audit_e97_pi_native_repair_generic.py --proposal "$PROP" \
  ...
  --schedule "$SCHED" \
  --output "$ST/proposal-audits/segment1-proposal-audit.json"

# ---- STEP 3: scoped commit + push of the frozen proposal
git add "$PROP"
git commit -m "Freeze repair-v9r full-arc segment 1 proposal"
...
bash "$ST/templates/instantiate-run-dir.sh" --segment 1 \
...
bash "$ST/templates/instantiate-juncture-dir.sh" --segment 1 --updates 128
# regenerate $P/pi-native-repair9r-full-arc-juncture-evals/seg1-u128/panel.json
# for the exact audited checkpoint, then:
# bash $P/pi-native-repair9r-full-arc-juncture-evals/seg1-u128/stage-juncture.sh
