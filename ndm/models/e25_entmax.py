"""
E25: Dual-Memory Elman with 1.5-Entmax Attention

Replaces softmax attention with 1.5-entmax for sparse attention patterns.
Based on E23 architecture.

1.5-entmax properties:
- Produces exact zeros for low-scoring slots (sparse)
- Closed-form: entmax_i(z) = [z_i - tau(z)]^2_+
- Threshold tau found via sorting and statistical computation
- Backward: dz[i] = 2 * sqrt(p[i]) * (dp[i] - v/g_sum) where v = sum(sqrt(p)*dp)
"""

import math
import torch
import torch.nn as nn
import torch.nn.functional as F


def entmax_1_5_forward(z: torch.Tensor, dim: int = -1) -> torch.Tensor:
    """
    1.5-entmax forward pass.

    Args:
        z: Input tensor of any shape
        dim: Dimension to apply entmax over

    Returns:
        p: Sparse probability distribution (same shape as z)
    """
    # Move working dimension to last
    z = z.transpose(dim, -1)
    original_shape = z.shape
    z = z.reshape(-1, z.shape[-1])  # [batch, N]

    batch_size, N = z.shape
    device = z.device
    dtype = z.dtype

    # Sort in descending order
    z_sorted, _ = torch.sort(z, dim=-1, descending=True)

    # Compute tau for each candidate support size k
    # For k=1,2,...,N, compute:
    #   mean_k = sum(z_sorted[:k]) / k
    #   ss_k = sum((z_sorted[:k] - mean_k)^2) / k^2
    #   delta_k = (1 - ss_k) / k
    #   tau_k = mean_k - sqrt(max(0, delta_k))

    # Cumulative sum for mean computation
    cumsum = torch.cumsum(z_sorted, dim=-1)  # [batch, N]
    k_vals = torch.arange(1, N + 1, device=device, dtype=dtype)  # [N]
    mean_k = cumsum / k_vals  # [batch, N]

    # Compute sum of squared deviations from mean
    # ss_k = sum_i^k (z_i - mean_k)^2 / k^2
    # Using: sum((x-m)^2) = sum(x^2) - 2*m*sum(x) + k*m^2
    #                     = sum(x^2) - k*m^2
    cumsum_sq = torch.cumsum(z_sorted ** 2, dim=-1)  # [batch, N]
    ss_k = (cumsum_sq - k_vals * mean_k ** 2) / (k_vals ** 2)  # [batch, N]

    # Compute delta and tau
    delta_k = (1.0 - ss_k) / k_vals  # [batch, N]
    tau_k = mean_k - torch.sqrt(torch.clamp(delta_k, min=0.0))  # [batch, N]

    # Find support size: k* = max{k : tau_k <= z_sorted[k-1]}
    # In other words, we want the largest k where the k-th sorted element is >= tau_k
    valid = tau_k <= z_sorted  # [batch, N]

    # Find largest valid k for each batch element
    # Multiply by k and take max
    k_star = (valid.float() * k_vals).max(dim=-1, keepdim=True).values  # [batch, 1]
    k_star = k_star.clamp(min=1)  # At least 1 element in support

    # Get tau for k_star
    # Use gather to select the right tau
    k_star_idx = (k_star - 1).long()  # Convert to 0-indexed
    tau_star = torch.gather(tau_k, dim=-1, index=k_star_idx)  # [batch, 1]

    # Apply transformation: q = max(0, z - tau)^2
    q = torch.clamp(z - tau_star, min=0.0) ** 2

    # Normalize: p = q / sum(q)
    Z = q.sum(dim=-1, keepdim=True).clamp(min=1e-9)
    p = q / Z

    # Reshape back
    p = p.reshape(original_shape)
    p = p.transpose(dim, -1)

    return p


def entmax_1_5_backward(p: torch.Tensor, dp: torch.Tensor, dim: int = -1) -> torch.Tensor:
    """
    1.5-entmax backward pass.

    Uses standard entmax gradient formula (same as deep-spin implementation):
    dz = g * (dp - (g @ dp) / sum(g))
    where g = sqrt(p)

    Note: This is an approximate gradient that treats tau as constant.
    This is standard practice and works well in training.

    Args:
        p: Forward output (sparse attention weights)
        dp: Gradient w.r.t. p
        dim: Dimension over which entmax was applied

    Returns:
        dz: Gradient w.r.t. input z
    """
    # Move working dimension to last
    p = p.transpose(dim, -1)
    dp = dp.transpose(dim, -1)
    original_shape = p.shape
    p = p.reshape(-1, p.shape[-1])
    dp = dp.reshape(-1, dp.shape[-1])

    # g = sqrt(p)
    g = torch.sqrt(torch.clamp(p, min=0.0))

    # Standard entmax gradient: dz = g * (dp - v) where v = (g @ dp) / sum(g)
    g_dp = (g * dp).sum(dim=-1, keepdim=True)
    g_sum = g.sum(dim=-1, keepdim=True).clamp(min=1e-9)
    dz = g * (dp - g_dp / g_sum)

    # Reshape back
    dz = dz.reshape(original_shape)
    dz = dz.transpose(dim, -1)

    return dz


class Entmax15Function(torch.autograd.Function):
    """Autograd function for 1.5-entmax."""

    @staticmethod
    def forward(ctx, z, dim=-1):
        p = entmax_1_5_forward(z, dim)
        ctx.save_for_backward(p)
        ctx.dim = dim
        return p

    @staticmethod
    def backward(ctx, dp):
        p, = ctx.saved_tensors
        dz = entmax_1_5_backward(p, dp, ctx.dim)
        return dz, None


def entmax_1_5(z: torch.Tensor, dim: int = -1) -> torch.Tensor:
    """
    Apply 1.5-entmax with autograd support.

    Args:
        z: Input scores
        dim: Dimension to normalize over

    Returns:
        Sparse probability distribution
    """
    return Entmax15Function.apply(z, dim)


def e25_forward_step_python(
    x: torch.Tensor,           # [B, D] - input at this timestep
    h_tape: torch.Tensor,      # [B, N, D] - tape state
    h_work: torch.Tensor,      # [B, D] - working memory state
    W_h: torch.Tensor,         # [D, D] - recurrence matrix
    W_x: torch.Tensor,         # [D, D] - input projection
    b_h: torch.Tensor,         # [D] - bias
    W_write: torch.Tensor,     # [D, D] - write projection
    scale: float,              # attention scale (1/sqrt(D))
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Single step of E25 forward pass (Python reference).
    Same as E23 but with 1.5-entmax instead of softmax.

    Returns:
        h_work_new: [B, D] - updated working memory
        h_tape_new: [B, N, D] - updated tape
        read_attn: [B, N] - read attention weights (sparse)
        write_attn: [B, N] - write attention weights (sparse)
        read: [B, D] - read value from tape
    """
    B, N, D = h_tape.shape

    # ============================================
    # STEP 1: WORKING MEMORY READS FROM TAPE
    # ============================================
    read_scores = (h_tape * h_work[:, None, :]).sum(dim=-1)  # [B, N]
    read_scores = read_scores * scale
    read_attn = entmax_1_5(read_scores, dim=-1)  # [B, N] - SPARSE

    # Weighted sum of tape slots
    read = (read_attn[:, :, None] * h_tape).sum(dim=1)  # [B, D]

    # ============================================
    # STEP 2: WORKING MEMORY UPDATE (Elman + read)
    # ============================================
    pre_act = (
        h_work @ W_h.T +
        x @ W_x.T +
        read +
        b_h
    )
    h_work_new = torch.tanh(pre_act)  # [B, D]

    # ============================================
    # STEP 3: WORKING MEMORY WRITES TO TAPE
    # ============================================
    write_value = h_work_new @ W_write.T  # [B, D]

    write_scores = (h_tape * h_work_new[:, None, :]).sum(dim=-1)  # [B, N]
    write_scores = write_scores * scale
    write_attn = entmax_1_5(write_scores, dim=-1)  # [B, N] - SPARSE

    # REPLACEMENT write
    h_tape_new = (
        (1 - write_attn[:, :, None]) * h_tape +
        write_attn[:, :, None] * write_value[:, None, :]
    )

    return h_work_new, h_tape_new, read_attn, write_attn, read


def e25_sequence_python(
    x_seq: torch.Tensor,       # [B, T, D] - input sequence
    h_tape: torch.Tensor,      # [B, N, D] - initial tape state
    h_work: torch.Tensor,      # [B, D] - initial working memory
    W_h: torch.Tensor,
    W_x: torch.Tensor,
    b_h: torch.Tensor,
    W_write: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, list, list]:
    """
    Process a sequence with E25 (Python reference).

    Returns:
        h_work_all: [B, T, D] - working memory at each timestep
        h_tape_final: [B, N, D] - final tape state
        h_work_final: [B, D] - final working memory
        read_attn_all: list of [B, N] - read attention at each step
        write_attn_all: list of [B, N] - write attention at each step
    """
    B, T, D = x_seq.shape
    N = h_tape.shape[1]
    scale = 1.0 / math.sqrt(D)

    outputs = []
    read_attn_all = []
    write_attn_all = []

    for t in range(T):
        h_work, h_tape, read_attn, write_attn, _ = e25_forward_step_python(
            x_seq[:, t], h_tape, h_work,
            W_h, W_x, b_h, W_write, scale
        )
        outputs.append(h_work)
        read_attn_all.append(read_attn)
        write_attn_all.append(write_attn)

    h_work_all = torch.stack(outputs, dim=1)  # [B, T, D]
    return h_work_all, h_tape, h_work, read_attn_all, write_attn_all


class E25DualMemoryElmanFunction(torch.autograd.Function):
    """Autograd function for E25 with entmax attention."""

    @staticmethod
    def forward(ctx, training, x_seq, h_tape_init, h_work_init,
                W_h, W_x, b_h, W_write, use_cuda=True):
        B, T, D = x_seq.shape
        N = h_tape_init.shape[1]
        scale = 1.0 / math.sqrt(D)

        # Check for CUDA kernel
        use_cuda_kernel = False
        if use_cuda and x_seq.is_cuda:
            try:
                import hasty_pytorch_lib
                if hasattr(hasty_pytorch_lib, 'e25_entmax_forward'):
                    use_cuda_kernel = True
            except ImportError:
                pass

        if use_cuda_kernel:
            # CUDA path
            import hasty_pytorch_lib
            h_work_all, h_tape_final, h_tape_all, read_attn_all, write_attn_all = \
                hasty_pytorch_lib.e25_entmax_forward(
                    training, x_seq, h_tape_init, h_work_init,
                    W_h, W_x, b_h, W_write
                )
            ctx.use_cuda = True
        else:
            # Python fallback
            h_tape = h_tape_init.clone()
            h_work = h_work_init.clone()

            h_work_list = []
            h_tape_list = [h_tape_init]
            read_attn_list = []
            write_attn_list = []

            for t in range(T):
                h_work, h_tape, read_attn, write_attn, _ = e25_forward_step_python(
                    x_seq[:, t], h_tape, h_work,
                    W_h, W_x, b_h, W_write, scale
                )
                h_work_list.append(h_work)
                h_tape_list.append(h_tape)
                read_attn_list.append(read_attn)
                write_attn_list.append(write_attn)

            h_work_all = torch.stack(h_work_list, dim=1)  # [B, T, D]
            h_tape_all = torch.stack(h_tape_list, dim=1)  # [B, T+1, N, D]
            read_attn_all = torch.stack(read_attn_list, dim=1)  # [B, T, N]
            write_attn_all = torch.stack(write_attn_list, dim=1)  # [B, T, N]
            h_tape_final = h_tape_all[:, -1]
            ctx.use_cuda = False

        ctx.save_for_backward(x_seq, h_work_all, h_tape_all, read_attn_all, write_attn_all,
                              W_h, W_x, b_h, W_write, h_work_init)
        ctx.scale = scale

        return h_work_all, h_tape_final

    @staticmethod
    def backward(ctx, d_h_work_all, d_h_tape_final):
        x_seq, h_work_all, h_tape_all, read_attn_all, write_attn_all, \
            W_h, W_x, b_h, W_write, h_work_init = ctx.saved_tensors
        scale = ctx.scale

        B, T, D = x_seq.shape
        N = h_tape_all.shape[2]

        if ctx.use_cuda:
            # CUDA backward - returns dx, dW_h, dW_x, db_h, dW_write
            # Note: pass x_seq (not x_proj) - CUDA kernel needs original x for dW_x computation
            import hasty_pytorch_lib
            dx, dW_h, dW_x, db_h, dW_write = \
                hasty_pytorch_lib.e25_entmax_backward(
                    x_seq.contiguous(), h_work_all, h_work_init.contiguous(), h_tape_all,
                    read_attn_all, write_attn_all,
                    W_h, W_x, W_write, d_h_work_all.contiguous(), d_h_tape_final.contiguous()
                )
        else:
            # Python backward
            dx, dW_h, dW_x, db_h, dW_write = e25_backward_python(
                x_seq, h_work_all, h_tape_all, read_attn_all, write_attn_all,
                W_h, W_x, b_h, W_write, h_work_init, d_h_work_all, d_h_tape_final, scale
            )

        return None, dx, None, None, dW_h, dW_x, db_h, dW_write, None


def e25_backward_python(
    x_seq, h_work_all, h_tape_all, read_attn_all, write_attn_all,
    W_h, W_x, b_h, W_write, h_work_init, d_h_work_all, d_h_tape_final, scale
):
    """
    Python reference backward pass for E25.
    Key difference from E23: uses entmax backward instead of softmax backward.
    """
    B, T, D = x_seq.shape
    N = h_tape_all.shape[2]

    # Initialize gradients
    dW_h = torch.zeros_like(W_h)
    dW_x = torch.zeros_like(W_x)
    db_h = torch.zeros_like(b_h)
    dW_write = torch.zeros_like(W_write)
    dx = torch.zeros_like(x_seq)

    d_h_tape = d_h_tape_final.clone()
    d_h_work = torch.zeros(B, D, device=x_seq.device, dtype=x_seq.dtype)

    for t in range(T - 1, -1, -1):
        h_work_t = h_work_all[:, t]
        h_tape_t = h_tape_all[:, t]
        read_attn = read_attn_all[:, t]
        write_attn = write_attn_all[:, t]

        if t > 0:
            h_work_prev = h_work_all[:, t - 1]
        else:
            h_work_prev = h_work_init

        d_h_work_t = d_h_work_all[:, t] + d_h_work

        # === BACKWARD THROUGH TAPE WRITE ===
        d_write_value = (d_h_tape * write_attn[:, :, None]).sum(dim=1)

        write_value = h_work_t @ W_write.T
        d_write_attn = (d_h_tape * (write_value[:, None, :] - h_tape_t)).sum(dim=-1)

        d_h_tape_pre_write = d_h_tape * (1 - write_attn[:, :, None])

        dW_write += d_write_value.T @ h_work_t
        d_h_work_from_write = d_write_value @ W_write

        # === ENTMAX BACKWARD FOR WRITE ATTENTION ===
        # d_write_scores = entmax_backward(write_attn, d_write_attn)
        d_write_scores = entmax_1_5_backward(write_attn, d_write_attn, dim=-1)
        d_write_scores = d_write_scores * scale

        d_h_work_from_write_attn = (d_write_scores[:, :, None] * h_tape_t).sum(dim=1)
        d_h_tape_from_write_attn = d_write_scores[:, :, None] * h_work_t[:, None, :]

        d_h_work_t_total = d_h_work_t + d_h_work_from_write + d_h_work_from_write_attn

        # === BACKWARD THROUGH WORKING MEMORY UPDATE ===
        d_pre_act = d_h_work_t_total * (1 - h_work_t ** 2)
        d_read = d_pre_act

        dW_h += d_pre_act.T @ h_work_prev
        d_h_work = d_pre_act @ W_h
        dW_x += d_pre_act.T @ x_seq[:, t]
        dx[:, t] = d_pre_act @ W_x
        db_h += d_pre_act.sum(dim=0)

        # === BACKWARD THROUGH READ ===
        d_read_attn = (d_read[:, None, :] * h_tape_t).sum(dim=-1)
        d_h_tape_from_read = d_read[:, None, :] * read_attn[:, :, None]

        # === ENTMAX BACKWARD FOR READ ATTENTION ===
        d_read_scores = entmax_1_5_backward(read_attn, d_read_attn, dim=-1)
        d_read_scores = d_read_scores * scale

        d_h_work_from_read_attn = (d_read_scores[:, :, None] * h_tape_t).sum(dim=1)
        d_h_work += d_h_work_from_read_attn
        d_h_tape_from_read_attn = d_read_scores[:, :, None] * h_work_prev[:, None, :]

        d_h_tape = d_h_tape_pre_write + d_h_tape_from_write_attn + d_h_tape_from_read + d_h_tape_from_read_attn

    return dx, dW_h, dW_x, db_h, dW_write


class E25DualMemoryElmanCell(nn.Module):
    """E25 Dual-Memory Elman Cell with 1.5-entmax attention."""

    def __init__(self, dim, n_slots=64, w_h_init_scale=0.9):
        super().__init__()
        self.dim = dim
        self.n_slots = n_slots

        self.W_h = nn.Parameter(torch.empty(dim, dim))
        self.W_x = nn.Parameter(torch.empty(dim, dim))
        self.b_h = nn.Parameter(torch.zeros(dim))
        self.W_write = nn.Parameter(torch.empty(dim, dim))

        self._init_weights(w_h_init_scale)

    def _init_weights(self, w_h_init_scale):
        W_h_fp32 = torch.empty_like(self.W_h, dtype=torch.float32)
        nn.init.orthogonal_(W_h_fp32)
        W_h_fp32.mul_(w_h_init_scale)
        with torch.no_grad():
            self.W_h.copy_(W_h_fp32.to(self.W_h.dtype))

        nn.init.xavier_uniform_(self.W_x)
        nn.init.xavier_uniform_(self.W_write)

    def forward(self, x_seq, h_tape=None, h_work=None, use_cuda=True):
        if x_seq.dim() == 3:
            if x_seq.shape[0] > x_seq.shape[1]:
                x_seq = x_seq.permute(1, 0, 2).contiguous()

        B, T, D = x_seq.shape

        if h_tape is None:
            h_tape = torch.zeros(B, self.n_slots, D, device=x_seq.device, dtype=x_seq.dtype)
        if h_work is None:
            h_work = torch.zeros(B, D, device=x_seq.device, dtype=x_seq.dtype)

        h_work_all, h_tape_final = E25DualMemoryElmanFunction.apply(
            self.training,
            x_seq.contiguous(),
            h_tape.contiguous(),
            h_work.contiguous(),
            self.W_h.contiguous(),
            self.W_x.contiguous(),
            self.b_h.contiguous(),
            self.W_write.contiguous(),
            use_cuda
        )

        h_work_final = h_work_all[:, -1]
        return h_work_all, h_tape_final, h_work_final


class E25DualMemoryElman(nn.Module):
    """
    E25: Dual-Memory Elman with 1.5-Entmax Attention.

    Same architecture as E23 but with sparse attention via 1.5-entmax.
    """

    def __init__(
        self,
        dim,
        expansion=1.0,
        n_slots=64,
        dropout=0.0,
        w_h_init_scale=0.9,
        mamba2_init=False,
        **kwargs
    ):
        super().__init__()
        self.dim = dim
        self.d_inner = int(dim * expansion)
        self.n_slots = n_slots

        self.in_proj = nn.Linear(dim, 2 * self.d_inner, bias=False)
        self.cell = E25DualMemoryElmanCell(
            self.d_inner,
            n_slots=n_slots,
            w_h_init_scale=w_h_init_scale
        )
        self.out_proj = nn.Linear(self.d_inner, dim, bias=False)
        self.dropout = nn.Dropout(dropout) if dropout > 0 else nn.Identity()

        self._init_weights(mamba2_init)

    def _init_weights(self, mamba2_init):
        if mamba2_init:
            nn.init.normal_(self.in_proj.weight, std=0.02)
            nn.init.normal_(self.out_proj.weight, std=0.02)
        else:
            nn.init.xavier_uniform_(self.in_proj.weight)
            nn.init.xavier_uniform_(self.out_proj.weight)

    def forward(self, x, h_tape=None, h_work=None, **kwargs):
        B, T, D = x.shape

        xz = self.in_proj(x)
        x_proj, z = xz.chunk(2, dim=-1)
        x_proj = F.silu(x_proj)

        h_work_all, h_tape_final, h_work_final = self.cell(x_proj, h_tape, h_work)

        output = h_work_all * F.silu(z)
        output = self.dropout(output)
        output = self.out_proj(output)

        return output, (h_tape_final, h_work_final)

    def extra_repr(self):
        return f'dim={self.dim}, d_inner={self.d_inner}, n_slots={self.n_slots}, LEVEL=25_ENTMAX'


if __name__ == "__main__":
    print("Testing E25 with 1.5-Entmax Attention...")
    print("=" * 60)

    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    dtype = torch.float32  # Use float32 for testing

    # Test entmax forward
    print("\n1. Testing 1.5-entmax forward...")
    z = torch.randn(4, 16, device=device, dtype=dtype)
    p = entmax_1_5(z, dim=-1)
    print(f"   Input shape: {z.shape}")
    print(f"   Output shape: {p.shape}")
    print(f"   Sum to 1: {p.sum(dim=-1)}")
    print(f"   Non-zero per row: {(p > 0).sum(dim=-1)}")
    print(f"   Sparsity: {(p == 0).float().mean().item():.2%}")

    # Test entmax backward
    print("\n2. Testing 1.5-entmax backward (gradient check)...")
    z = torch.randn(2, 8, device=device, dtype=torch.float64, requires_grad=True)

    from torch.autograd import gradcheck
    try:
        result = gradcheck(lambda x: entmax_1_5(x, dim=-1), z, eps=1e-6, atol=1e-4)
        print(f"   Gradient check: {'PASS' if result else 'FAIL'}")
    except Exception as e:
        print(f"   Gradient check failed: {e}")

    # Test E25 cell
    print("\n3. Testing E25DualMemoryElmanCell...")
    B, T, D = 2, 32, 128
    N_SLOTS = 16

    cell = E25DualMemoryElmanCell(dim=D, n_slots=N_SLOTS).to(device).to(dtype)
    x = torch.randn(B, T, D, device=device, dtype=dtype)

    h_work_all, h_tape_final, h_work_final = cell(x, use_cuda=False)
    print(f"   Input: {x.shape}")
    print(f"   h_work_all: {h_work_all.shape}")
    print(f"   h_tape_final: {h_tape_final.shape}")

    # Test backward
    print("\n4. Testing backward pass...")
    loss = h_work_all.sum()
    loss.backward()
    print(f"   W_h grad norm: {cell.W_h.grad.norm().item():.4f}")
    print(f"   W_x grad norm: {cell.W_x.grad.norm().item():.4f}")
    print(f"   W_write grad norm: {cell.W_write.grad.norm().item():.4f}")

    # Test full layer
    print("\n5. Testing E25DualMemoryElman layer...")
    cell.zero_grad()
    layer = E25DualMemoryElman(dim=D, expansion=1.0, n_slots=N_SLOTS).to(device).to(dtype)
    output, (h_tape, h_work) = layer(x)
    print(f"   Output: {output.shape}")

    loss = output.sum()
    loss.backward()
    print(f"   in_proj grad norm: {layer.in_proj.weight.grad.norm().item():.4f}")

    # Compare sparsity with E23
    print("\n6. Attention sparsity analysis...")
    cell2 = E25DualMemoryElmanCell(dim=64, n_slots=32).to(device).to(dtype)
    x2 = torch.randn(4, 16, 64, device=device, dtype=dtype)

    with torch.no_grad():
        h_work_all, h_tape_final, h_work_final, read_attn, write_attn = e25_sequence_python(
            x2,
            torch.zeros(4, 32, 64, device=device, dtype=dtype),
            torch.zeros(4, 64, device=device, dtype=dtype),
            cell2.W_h, cell2.W_x, cell2.b_h, cell2.W_write
        )

    read_attn_stack = torch.stack(read_attn)
    write_attn_stack = torch.stack(write_attn)

    read_sparsity = (read_attn_stack == 0).float().mean().item()
    write_sparsity = (write_attn_stack == 0).float().mean().item()
    read_nonzero = (read_attn_stack > 0).sum(dim=-1).float().mean().item()
    write_nonzero = (write_attn_stack > 0).sum(dim=-1).float().mean().item()

    print(f"   Read attention sparsity: {read_sparsity:.2%}")
    print(f"   Write attention sparsity: {write_sparsity:.2%}")
    print(f"   Avg non-zero read weights: {read_nonzero:.1f}/{32}")
    print(f"   Avg non-zero write weights: {write_nonzero:.1f}/{32}")

    print("\n" + "=" * 60)
    print("E25 Python reference implementation test passed!")
