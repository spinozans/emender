...
Architecture (same as E1, but with softsign activation):
    ...
    h_t = softsign(W_x @ x_t + W_h @ h_{t-1} + b)  # Softsign recurrence
...
try:
    ...
    SOFTSIGN_CUDA_AVAILABLE = hasattr(hasty_pytorch_lib, 'softsign_elman_forward')
except ImportError:
    SOFTSIGN_CUDA_AVAILABLE = False
...
class SoftsignElmanFunction(torch.autograd.Function):
    """CUDA-accelerated softsign elman autograd function."""

    @staticmethod
    def forward(ctx, training, x, z, h0, W_x, W_h, b):
        h, output, v = hasty_pytorch_lib.softsign_elman_forward(
    ...
    def backward(ctx, dh, d_output):
        ...
        dx, dz, dW_x, dW_h, db = hasty_pytorch_lib.softsign_elman_backward(
            W_x, W_h, x, z, h, v, d_output.contiguous()
        )
        return None, dx, dz, None, dW_x, dW_h, db


class SoftsignElmanCell(nn.Module):
    """
    Elman cell with softsign activation and Mamba2-style gating.
    ...
    h_t = softsign(W_x @ x_t + W_h @ h_{t-1} + b)
    output = h_t * silu(z_t)
    """
    ...
    def forward(self, x, z, h0=None):
        ...
        if SOFTSIGN_CUDA_AVAILABLE and x.is_cuda:
            h, output = SoftsignElmanFunction.apply(
                self.training,
                x.contiguous(),
                z.contiguous(),
                h0.contiguous(),
        ...
        for t in range(T):
            ...
            # Elman recurrence with softsign
            raw = x_t @ self.W_x.T + h_prev @ W_h.T + self.b
            h_new = softsign(raw)  # softsign instead of tanh
            h_list.append(h_new)
...
class SoftsignElman(nn.Module):
    """
    Softsign Elman layer with Mamba2-style split projection.

    Architecture:
        x, z = split(in_proj(x))    # Split into RNN input and gate
        x = conv1d(x) if use_conv   # Optional local context
        x = silu(x)                 # Pre-activation
        h = elman_cell(x, z)        # RNN with softsign and gated output
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Elman cell with softsign
        self.cell = SoftsignElmanCell(
            self.d_inner,
            w_h_mode=r_h_mode,
    ...
    def forward(self, x, h0=None, **kwargs):
        ...
        # Run Elman cell with softsign and Mamba2-style gating
    ...
    def extra_repr(self):
        return f'dim={self.dim}, d_inner={self.d_inner}, use_conv={self.use_conv}, ACTIVATION=softsign'


if __name__ == "__main__":
    print("Testing SoftsignElman...")
    print("=" * 60)

    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    # Test without conv
    model = SoftsignElman(dim=512, expansion=2.0, use_conv=False).to(device).bfloat16()
    ...
    # Test with conv
    model_conv = SoftsignElman(dim=512, expansion=2.0, use_conv=True, d_conv=4).to(device).bfloat16()
