...
Architecture (matching E33 but with cubic gating):
    x = in_proj(x)                     # No split needed
    x = silu(x)                        # Pre-activation
    h_t = tanh(W_x @ x_t + W_h @ h_{t-1} + b)  # Elman recurrence
    output = h^3                       # Cubic gating: h cubed

Key differences from E33:
    - E33: output = h * silu(h)  -- self-gating with silu
    - E35: output = h^3          -- simpler cubic gating

Gradient: d(h^3)/dh = 3h^2
"""
...
try:
    ...
    E35_CUDA_AVAILABLE = hasattr(hasty_pytorch_lib, 'e35_cubic_gate_forward')
except ImportError:
    E35_CUDA_AVAILABLE = False


class E35CubicGateFunction(torch.autograd.Function):
    """CUDA-accelerated E35 cubic-gated elman autograd function."""

    @staticmethod
    def forward(ctx, training, x, h0, W_x, W_h, b):
        # E35: no z input - output = h^3
        h, output, v = hasty_pytorch_lib.e35_cubic_gate_forward(
    ...
    def backward(ctx, dh, d_output):
        ...
        # E35: no dz output
        dx, dW_x, dW_h, db = hasty_pytorch_lib.e35_cubic_gate_backward(
            W_x, W_h, x, h, v, d_output.contiguous()
        )
        return None, dx, None, dW_x, dW_h, db


class E35CubicGateCell(nn.Module):
    """
    E35 Cubic-Gated Elman cell - simplification test.

    Key difference from E33: output = h^3 instead of h * silu(h)
    This is even simpler than self-gating.

    h_t = tanh(W_x @ x_t + W_h @ h_{t-1} + b)
    output = h_t^3   # Cubic gating
    """
    ...
    def _init_weights(self, w_h_init_gain):
        if self.mamba2_init == 's4d':
            ...
            # Build diagonal: W_h = Q @ diag(radii) @ Q.T
            D = torch.diag(radii)
            W_h_fp32 = Q @ D @ Q.T
    ...
    def forward(self, x, z=None, h0=None):
        ...
        Args:
            ...
            z: unused (kept for API compatibility, E35 uses h^3)
        ...
        if E35_CUDA_AVAILABLE and x.is_cuda:
            h, output = E35CubicGateFunction.apply(
                self.training,
                x.contiguous(),
                h0.contiguous(),
                self.W_x.contiguous(),
        ...
        # PyTorch fallback with cubic gating
        h_list = [h0]
        output_list = []
        ...
        for t in range(T):
            ...
            # E35 CUBIC GATING: output = h^3 instead of h * silu(h)
            output = h_new ** 3
...
class E35CubicGate(nn.Module):
    """
    E35: Cubic-Gated Elman layer with h^3 output gating.

    Architecture:
        x = in_proj(x)              # Project input
        x = silu(x)                 # Pre-activation
        h = elman_cell(x)           # RNN with cubic gated output
    ...
    def __init__(
        ...
        **kwargs
        ...
        # E35: project to 1*d_inner only (no z split needed for cubic gating)
    ...
    def forward(self, x, h0=None, **kwargs):
        ...
        # E35: no split needed (cubic gating uses h^3)
        ...
        # E35: Run cell with cubic gating (z unused, pass None)
    ...
    def extra_repr(self):
        return f'dim={self.dim}, d_inner={self.d_inner}, use_conv={self.use_conv}, LEVEL=35_CUBIC_GATE'


if __name__ == "__main__":
    print("Testing E35CubicGate...")
    print("=" * 60)

    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    # Test without conv
    model = E35CubicGate(dim=512, expansion=2.0, use_conv=False).to(device).bfloat16()
    ...
    # Test with conv
    model_conv = E35CubicGate(dim=512, expansion=2.0, use_conv=True, d_conv=4).to(device).bfloat16()
    ...
    # Verify gradient: d(h^3)/dh = 3h^2
    print("\nVerifying cubic gradient...")
    h_test = torch.randn(10, device=device, dtype=torch.float32, requires_grad=True)
    out_test = h_test ** 3
    out_test.sum().backward()
    expected_grad = 3 * h_test.detach() ** 2
    print(f"Gradient check: {torch.allclose(h_test.grad, expected_grad)}")

    print("\nE35 (Cubic-Gated Elman) test passed!")
