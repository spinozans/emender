...
Architecture:
    ...
    h_t = tanh(x_proj + W_h @ h_{t-1} + b)  # No W_x, raw x_proj
    output = h_t * silu(h_t)         # Self-gating

Key differences from E33:
    - E33: x_proj = silu(in_proj(x)), then W_x @ x_proj + W_h @ h + b
    - E40: x_proj = in_proj(x), then x_proj + W_h @ h + b (no silu, no W_x)

Like E38 but without the pre-silu activation.
This tests: with W_x removed, is pre-silu still needed?
"""
...
try:
    ...
    E40_CUDA_AVAILABLE = hasattr(hasty_pytorch_lib, 'e40_no_presilu_forward')
except ImportError:
    E40_CUDA_AVAILABLE = False


class E40NoPresiluFunction(torch.autograd.Function):
    """CUDA-accelerated E40 no-presilu elman autograd function."""

    @staticmethod
    def forward(ctx, training, x, h0, W_h, b):
        # E40: no W_x, x goes directly into recurrence
        h, output, v = hasty_pytorch_lib.e40_no_presilu_forward(
            training, x, h0, W_h, b
        )
    ...
    def backward(ctx, dh, d_output):
        ...
        # E40: no dW_x output (no W_x parameter)
        dx, dW_h, db = hasty_pytorch_lib.e40_no_presilu_backward(
            W_h, x, h, v, d_output.contiguous()
        )
        return None, dx, None, dW_h, db


class E40NoPresiluCell(nn.Module):
    """
    E40 No Pre-SiLU Elman cell - E38 without pre-activation.

    Key differences from E33:
    - No W_x (removed in E38)
    - No pre-silu (removed in E40)

    h_t = tanh(x_t + W_h @ h_{t-1} + b)   # x_t is NOT pre-activated
    output = h_t * silu(h_t)              # Self-gating
    """
    ...
    def forward(self, x, z=None, h0=None):
        """
        Args:
            x: [T, B, dim] input for RNN (NOT pre-activated)
        ...
        if E40_CUDA_AVAILABLE and x.is_cuda:
            h, output = E40NoPresiluFunction.apply(
                self.training,
                x.contiguous(),
                h0.contiguous(),
                W_h.contiguous(),
        ...
        # PyTorch fallback with self-gating
        h_list = [h0]
        output_list = []
        ...
        for t in range(T):
            ...
            # E40: Elman recurrence WITHOUT W_x (x goes directly)
...
class E40NoPresilu(nn.Module):
    """
    E40: No Pre-SiLU Elman layer - E38 without pre-activation silu.

    Architecture:
        x_proj = in_proj(x)              # No silu! Direct projection
        x_proj = conv1d(x_proj) if conv  # Optional local context
        h = elman_cell(x_proj)           # RNN: tanh(x + Rh + b), output = h * silu(h)
    ...
    def __init__(
        ...
        **kwargs
        ...
        # E40: project to d_inner (no z split, no silu after)
    ...
    def forward(self, x, h0=None, **kwargs):
        ...
        # E40: project WITHOUT silu (key difference from E33/E38)
        ...
        # E40: NO pre-silu! x_proj goes directly to cell
    ...
    def extra_repr(self):
        return f'dim={self.dim}, d_inner={self.d_inner}, use_conv={self.use_conv}, LEVEL=40_NO_PRESILU'


if __name__ == "__main__":
    print("Testing E40NoPresilu...")
    print("=" * 60)

    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    # Test without conv
    model = E40NoPresilu(dim=512, expansion=2.0, use_conv=False).to(device).bfloat16()
    ...
    # Test with conv
    model_conv = E40NoPresilu(dim=512, expansion=2.0, use_conv=True, d_conv=4).to(device).bfloat16()
    ...
    print("\nE40 (No Pre-SiLU Elman) test passed!")
