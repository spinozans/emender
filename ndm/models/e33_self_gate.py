...
try:
    ...
    E33_CUDA_AVAILABLE = hasattr(hasty_pytorch_lib, 'e33_self_gate_forward')
except ImportError:
    E33_CUDA_AVAILABLE = False


class E33SelfGateFunction(torch.autograd.Function):
    """CUDA-accelerated E33 self-gated elman autograd function."""

    @staticmethod
    def forward(ctx, training, x, h0, W_x, W_h, b):
        # E33: no z input - h gates itself
        h, output, v = hasty_pytorch_lib.e33_self_gate_forward(
    ...
    def backward(ctx, dh, d_output):
        ...
        # E33: no dz output
        dx, dW_x, dW_h, db = hasty_pytorch_lib.e33_self_gate_backward(
            W_x, W_h, x, h, v, d_output.contiguous()
        )
        return None, dx, None, dW_x, dW_h, db


class E33SelfGateCell(nn.Module):
    """
    E33 Self-Gated Elman cell - simplification test.

    Key difference from E1: h gates itself instead of using separate z branch.
    This eliminates the z projection entirely, halving in_proj parameters.
    ...
    def forward(self, x, z=None, h0=None):
        ...
        Args:
            ...
            z: unused (kept for API compatibility, E33 gates h with h)
        ...
        if E33_CUDA_AVAILABLE and x.is_cuda:
            h, output = E33SelfGateFunction.apply(
                self.training,
                x.contiguous(),
                h0.contiguous(),
                self.W_x.contiguous(),
        ...
        for t in range(T):
            ...
            # E33 SELF-GATING: output = h * silu(h) instead of h * silu(z)
...
class E33SelfGate(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # E33: project to 1*d_inner only (no z split needed for self-gating)
    ...
    def forward(self, x, h0=None, **kwargs):
        ...
        # E33: no split needed (self-gating uses h, not z)
        ...
        # E33: Run cell with self-gating (z unused, pass None)
    ...
    def extra_repr(self):
        return f'dim={self.dim}, d_inner={self.d_inner}, use_conv={self.use_conv}, LEVEL=33_SELF_GATE'


if __name__ == "__main__":
    print("Testing E33SelfGate...")
    print("=" * 60)

    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    # Test without conv
    model = E33SelfGate(dim=512, expansion=2.0, use_conv=False).to(device).bfloat16()
    ...
    # Test with conv
    model_conv = E33SelfGate(dim=512, expansion=2.0, use_conv=True, d_conv=4).to(device).bfloat16()
    ...
    print("\nE33 (Self-Gated Elman) test passed!")
